...
`Challenge.lean` states twelve theorems over Mathlib alone, and `comparator.json` selects exactly
...
`Classical.choice` and `Quot.sound`, and the twelve proved statements use exactly those three
...
declaration, and that the twelve headline names agree across the challenge, solution,
