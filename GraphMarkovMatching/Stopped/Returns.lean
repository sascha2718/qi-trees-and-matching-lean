...
/-- The phase along a possible path: position `k` has phase `θ (p 0) + k` (`sec:return-times`). -/
...
/-- The residue modulo `g` of an element of `Γ` vanishes (`sec:return-times`). -/
...
element of `Γ` (`sec:return-times`: a leaf depth of a composite profile is a sum of core leaf
depths). -/
...
/-- Every leaf depth of a composite profile lies in `Γ` (`sec:return-times`: a sum of core leaf
depths). -/
...
/-- All leaf depths of a live remaining tree are congruent modulo `g` (`sec:return-times`). -/
...
/-- The phase of a live type (`sec:return-times`): `0` at a fresh type, `-d (mod g)` at a forced
...
(`sec:return-times`: each child has phase one greater than its parent). -/
...
/-- **The phase map** (`sec:return-times`): fresh types have phase `0`, children have phase one
greater. -/
...
/-! ### Possible paths of a presentation (`sec:return-times`) -/
...
(`sec:return-times`: choose a leaf of the remaining tree). -/
...
steps (`sec:return-times`: gaps between fresh visits are at most `ℓ`). -/
...
(`sec:return-times`: choose core arities and follow a leaf of the required depth). -/
...
/-- The explicit return-threshold bound of `sec:return-times`: with `a₀ = min R_0/g` and
