...
* `delta3_e4_row`: the fresh-tilt screen row: the fresh tilt converts
  across the branch point with the root factor `r_μ(v0)^{-α}`
  (`WresD_Tlaw_succ_branch`), the survivor split of the collapsed
  mixture tilt gives the two pairings, and each pairing prunes to a
  quadratic product of counter-`2` screens;
...
/-! ### The fresh-tilt screen row -/

/-- **Forced-source screen row, fresh tilt** (`sec:rows` at
`ν = δ₃`, screen rows, entry `E₄`): the fresh dead indicator at the
counter-`2` cell descends to the collapsed mixture, the fresh tilt
converts with the root factor `r_μ(v0)^{-α}`, and the survivor split
with diagonal pruning leaves the two quadratic products of
counter-`2` screens. -/
theorem delta3_e4_row (hα : 1 ≤ α) (hrefl : ∀ v, Rv v v)
  ...
  -- diagonal pruning: every factorized output whose zero list contains
  -- the fresh law vanishes, so only the `[Z₂]`-column product survives
  have hfac : ∀ G₀ G₁ : FullLab (V × ℕ) h → ℝ≥0∞,
    ...
    ring
  -- the fresh screen row at the cell root `(v0, 2)`
  have hdesc : screenE (Zlaw μ (PMF.pure 3) v0 2 (h + 1))
    ...
    exact screenG_Tlaw_succ Rv μ (PMF.pure 3) v0 v0 hvpos 2 h
      (WresD α (Tlaw μ (PMF.pure 3) v0 (h + 1))
        (fullSim (labRel Rv) (h + 1)))
  -- the fresh tilt converts, producing the root factor
  have htilt : ∀ xp : FullLab (V × ℕ) h × FullLab (V × ℕ) h,
    ...
    intro xp
    rw [WresD_Tlaw_succ_branch α Rv μ (PMF.pure 3) v0 v0 hvpos 2 h xp,
      XiBar_delta3 μ v0 h]
    ring
  -- the survivor split of the collapsed mixture tilt
  have hpt : ∀ xp : FullLab (V × ℕ) h × FullLab (V × ℕ) h,
      prodPMF (Tlaw μ (PMF.pure 3) v0 h) (Tlaw μ (PMF.pure 3) v0 h) xp
          * ((if rE (prodPMF (Zlaw μ (PMF.pure 3) v0 2 h)
                  (Tlaw μ (PMF.pure 3) v0 h))
                (SquareRel (fullSim (labRel Rv) h)) xp = 0 then 1 else 0)
            * WresD α (prodPMF (Zlaw μ (PMF.pure 3) v0 2 h)
                  (Tlaw μ (PMF.pure 3) v0 h))
    ...
    exact mul_le_mul_right (mul_le_mul_right
      (WresD_square_le_sum hα0 (Zlaw μ (PMF.pure 3) v0 2 h)
        (Tlaw μ (PMF.pure 3) v0 h) (fullSim (labRel Rv) h) xp) _) _
  rw [hdesc, tsum_congr htilt, ENNReal.tsum_mul_left,
  ...
  rw [ENNReal.tsum_add]
  exact add_le_add
    (hfac (WresD α (Zlaw μ (PMF.pure 3) v0 2 h) (fullSim (labRel Rv) h))
      (WresD α (Tlaw μ (PMF.pure 3) v0 h) (fullSim (labRel Rv) h)))
