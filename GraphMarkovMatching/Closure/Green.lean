...
/-- Explicit denominator-free version of `rareMatrix_resolvent_le`. -/
theorem rareMatrix_resolvent_le_graftGreen
    {N M : iota → iota → ℝ≥0∞} {zeta C : ℝ≥0∞} {r : ℕ}
    (hr : 0 < r) (hzeta : zeta ≤ 1)
    (hNrow : ∀ i, ∑ j, N i j ≤ C)
    (hMrow : ∀ i, ∑ j, M i j ≤ C)
    (hnil : (mulVec N)^[r] (fun _ => 1) = fun _ => 0)
    (hhalf : zeta * (2 * C) ^ r ≤ 2⁻¹) (i : iota) :
    infResolvent (rareMatrix N M zeta) (fun _ => 1) i ≤
      graftGreenMultiplier C r := by
  refine (rareMatrix_resolvent_le hr hzeta hNrow hMrow hnil i).trans ?_
  rw [graftGreenMultiplier, graftGreenRemainder]
  simpa [mul_comm] using mul_le_mul_right
    (graft_geometric_denominator_le_two hhalf)
    (∑ j : Fin r, (2 * C) ^ (j : ℕ))
