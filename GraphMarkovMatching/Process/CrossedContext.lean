/-
The renewal-grammar layer for the varying-offspring process (from the
retired draft's arbitrary-`ν` section): the certified building blocks of
the early block recursion, and the obstruction that dictated the current
design, `thm:obstruction` of `arbitrary_offspring_matching.tex`
(`crossed_context_potential_top` below is its certificate).

Positive results:

* `Zlaw_pair_succ_eq`: for same-counter forced contexts the root factor is
  free (reflexivity at the forced label), so the pair potential at height
  `h+1` is exactly the child-pair potential at height `h`;
* `Zlaw_two_pair_le` (**the fresh cherry**): counter-`2` contexts resolve
  into fresh products in one step, so their pair potential contracts
  through the certified four-law, `Φ(Z₂-pair) ≤ A·Θ_h + C·Θ_h²`.
...
* `crossed_context_potential_top`: for the pure-ternary law (`ν` supported
  ...
  This is the varying-offspring analogue of `caret_etaD_eq_top`.
-/
...
/-! ### Same-counter forced pairs: the root factor is free -/

/-- For same-counter forced contexts the pair potential steps down exactly to
the child-pair potential: the forced roots are compatible by reflexivity. -/
lemma Zlaw_pair_succ_eq (hrefl : Rv v0 v0) (k h : ℕ) :
    PhiD α (Zlaw μ ν v0 k (h + 1)) (Zlaw μ ν v0 k (h + 1))
        (fullSim (labRel Rv) (h + 1))
      = PhiD α (Xi μ ν v0 k h) (Xi μ ν v0 k h)
          (SquareRel (fullSim (labRel Rv) h)) := by
  have hq : ∀ xp, q ((Xi μ ν v0 k h).map (branch (v0, k)))
        (fullSim (labRel Rv) (h + 1)) (branch (v0, k) xp)
      = q (Xi μ ν v0 k h) (SquareRel (fullSim (labRel Rv) h)) xp := by
    intro xp
    rw [q, q, show (Xi μ ν v0 k h).map (branch (v0, k))
        = muM (varyK μ ν v0) (v0, k) (h + 1) from rfl,
      qE_succ_branch (varyK μ ν v0) (labRel Rv)
        (show labRel Rv (v0, k) (v0, k) from hrefl) h xp]
    rfl
  rw [show Zlaw μ ν v0 k (h + 1) = (Xi μ ν v0 k h).map (branch (v0, k)) from rfl,
    PhiD_map_left, PhiD]
  exact tsum_congr fun xp => by rw [hq xp]

/-- **The fresh cherry**: counter-`2` context pairs contract through the
four-law at the fresh product reference. -/
theorem Zlaw_two_pair_le {δ L K : ℝ} (hα : 1 ≤ α) (hδ : 0 < δ)
    (hL0 : 0 ≤ L) (hL : ∀ t : ℝ, 0 ≤ t → t / (1 + t) ^ α ≤ L)
    (hK0 : 0 < K) (hK : ∀ t : ℝ, 0 ≤ t → t ≤ 1 → t * (1 - t) ^ α ≤ K)
    (hsymm : ∀ a b, Rv a b → Rv b a) (hrefl : Rv v0 v0) (h : ℕ) :
    PhiD α (Zlaw μ ν v0 2 (h + 1)) (Zlaw μ ν v0 2 (h + 1))
        (fullSim (labRel Rv) (h + 1))
      ≤ ENNReal.ofReal (2 * L + 2 * (1 + δ) * K) * Theta α Rv μ ν v0 h
        + ENNReal.ofReal (2 * L * chordConst α + 20 + 2 * (1 + δ⁻¹) * α ^ 2)
          * Theta α Rv μ ν v0 h ^ 2 := by
  rw [Zlaw_pair_succ_eq α Rv μ ν v0 hrefl 2 h, Xi_of_le_two μ ν v0 (le_refl 2) h]
  exact prodT_square_le α Rv μ ν v0 hα hδ hL0 hL hK0 hK hsymm h
