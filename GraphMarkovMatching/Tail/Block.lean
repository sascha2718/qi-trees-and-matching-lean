/-
The abstract infinite-index closure used by the exponential-tail extension of
`arbitrary_offspring_matching.tex` (`thm:exponential-tail-matching`).

The finite-support engine in `Closure/Block.lean` obtains its screen
invariant from a finite nilpotent matrix.  For an infinite support the
concrete screen index is countable and only locally nilpotent.  The graded
path expansion is used to construct a finite invariant `F` directly.  Once
such an invariant is available, neither finiteness of the index nor a matrix
presentation is needed.  `screened_uniform_bound_invariant` certifies
precisely that abstract closure step.

The first part below supplies countable-kernel algebra.  The second part gives
two abstract closure theorems, including the positive-functional interface
for a weighted transfer.  The former claimed counterexample to a uniform
seed resolvent is removed by hybrid component-in-mixture pruning, and the
regular hybrid remainder is now reduced to ordinary restricted potentials.
No concrete weighted ordinary/screen operator and finite functional
invariant have been formalised; consequently this file does not certify the
exponential-tail conjecture.
-/
