...
This file fixes the definitions and the structural facts:

* `SquareRel`      the symmetrised square relation on `X × X`
* refl / symm      preserved from `R`, as required to feed `R^□` back into
                   the contraction
* `aOverlap`       `a = μ{y : x₀ R y ∧ x₁ R y}`, the both-compatible overlap
* `cOverlap`       `c = μ{y : x₀ ⋡ y ∧ x₁ ⋡ y}`, the neither-compatible mass
* `aOverlap_le_rE` `a ≤ r_i`, since a single `Y` compatible with both is in
                   particular compatible with each (so `a ≤ min(r₀,r₁)`)
...
lemma SquareRel_refl {R : X → X → Prop} (h : ∀ x, R x x) (x : X × X) :
    SquareRel R x x :=
  Or.inl ⟨h x.1, h x.2⟩

lemma SquareRel_symm {R : X → X → Prop} (h : ∀ a b, R a b → R b a) (x y : X × X) :
    SquareRel R x y → SquareRel R y x := by
  rintro (⟨h₁, h₂⟩ | ⟨h₁, h₂⟩)
  · exact Or.inl ⟨h _ _ h₁, h _ _ h₂⟩
  · exact Or.inr ⟨h _ _ h₂, h _ _ h₁⟩
...
compatible with `x₀`. Half of `a ≤ min(r₀,r₁)`. -/
...
/-- `c` is the bad-degree overlap: it equals `μ` of the set where the two
"columns" of the transversal array are both bad. This form (with the negations
as a single `if`) is what the union bound in M6b will consume. -/
