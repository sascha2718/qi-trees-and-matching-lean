...
/-- The first letter of an address of a forest names the tree it belongs to. -/
lemma head_lt_length_of_mem_addrListF : ∀ {F : List RTree} {i : ℕ} {w : List ℕ},
    i :: w ∈ addrListF F → i < F.length
  | c :: cs, i, w, h => by
      rw [addrListF_cons, List.mem_append] at h
      rcases h with h | h
      · obtain ⟨v, -, hv⟩ := List.mem_map.mp h
        simp only [List.cons.injEq] at hv
        simp [← hv.1]
      · obtain ⟨v, hv, hveq⟩ := List.mem_map.mp h
        have hne := ne_nil_of_mem_addrListF hv
        cases v with
        | nil => exact absurd rfl hne
        | cons j v =>
            simp only [bump_cons, List.cons.injEq] at hveq
            have := head_lt_length_of_mem_addrListF (F := cs) (i := j) (w := v) hv
...
/-- The addresses of a forest are listed once each. -/
lemma nodup_addrListF (F : List RTree) : (addrListF F).Nodup :=
  nodup_addrListF_of F fun c _ => nodup_addrList c
