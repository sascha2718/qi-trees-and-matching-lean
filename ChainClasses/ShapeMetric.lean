...
/-- The same count, in terms of the neck length `m`. -/
lemma dist_entry_exit_add_one (σ : Shape) :
    dist (shapeSpace σ).entry (shapeSpace σ).exit + 1 = (σ.neckLen : ℝ) := by
  rw [dist_entry_exit, Shape.neckLen]
  push_cast
  ring
