...
* `NetAdj`, `NetLink`: the edge of the label graph `G_D` of `def:shape-net`
  ...
  the base case as hypotheses; on shapes the base case is
  `Shape.eq_of_size_le_one` and the shrinking step is the leaf deletion
  `Tri.del` read as a `1`-marked quasi-isometry.
