...
* `surviveCoeff_zero` (`= f(q) = q`, the extinction event that conditioning
  ...
  transform is a law), `tilde_nonneg`.
...
* `conjugate_sum`, `conjugate_mean`: the conjugate law `eq:conjugate-general`
  is a law (`∑_j θ_j q^{j-1} = f(q)/q = 1`) with mean `f'(q)`.
* `extinction_ge` (`q ≥ θ₀ > 0`) and `genDeriv_pos` with
  `exists_pos_of_supercritical` (`f'(q) > 0`); the subcriticality `f'(q) < 1`
...
lemma surviveCoeff_nonneg (J : ℕ) (θ : ℕ → ℝ) {q : ℝ} (hnn : ∀ j, 0 ≤ θ j)
    (hq0 : 0 ≤ q) (hq1 : q ≤ 1) (k : ℕ) : 0 ≤ surviveCoeff J θ q k :=
  Finset.sum_nonneg fun j _ =>
    mul_nonneg (mul_nonneg (mul_nonneg (hnn j) (Nat.cast_nonneg _))
      (pow_nonneg (by linarith) _)) (pow_nonneg hq0 _)
...
lemma tilde_nonneg (J : ℕ) (θ : ℕ → ℝ) {q : ℝ} (hnn : ∀ j, 0 ≤ θ j)
    (hq0 : 0 ≤ q) (hq1 : q < 1) (k : ℕ) : 0 ≤ tilde J θ q k := by
  rw [tilde]
  split
  · exact le_refl 0
  · exact div_nonneg (surviveCoeff_nonneg J θ hnn hq0 hq1.le k) (by linarith)
...
/-- For `j ≥ 1` the quotient is the tilt `θ_j q^{j-1}` of the paper. -/
lemma conjugateLaw_eq {θ : ℕ → ℝ} {q : ℝ} (hq : q ≠ 0) {j : ℕ} (hj : 1 ≤ j) :
    conjugateLaw θ q j = θ j * q ^ (j - 1) := by
  obtain ⟨n, rfl⟩ := Nat.exists_eq_add_of_le hj
  rw [conjugateLaw, show (1 + n) - 1 = n by omega, pow_add, pow_one]
  field_simp
...
/-- Supercriticality forces positive mass at some `j ≥ 1`. -/
lemma exists_pos_of_supercritical (J : ℕ) (θ : ℕ → ℝ) (hnn : ∀ j, 0 ≤ θ j)
    (hmean : 1 < ∑ j ∈ Finset.range (J + 1), (j : ℝ) * θ j) :
    ∃ j ∈ Finset.range (J + 1), 1 ≤ j ∧ 0 < θ j := by
  by_contra hc
  push Not at hc
  have hz : ∀ j ∈ Finset.range (J + 1), (j : ℝ) * θ j = 0 := by
    intro j hj
    rcases Nat.eq_zero_or_pos j with rfl | hj1
    · simp
    · have hle := hc j hj hj1
      have hzj : θ j = 0 := le_antisymm hle (hnn j)
      rw [hzj, mul_zero]
  rw [Finset.sum_congr rfl hz] at hmean
  simp at hmean
  linarith
...
lemma genFun_hairy (θ₀ θ₁ θ₂ s : ℝ) :
    genFun 2 (hairyLaw θ₀ θ₁ θ₂) s = θ₀ + θ₁ * s + θ₂ * s ^ 2 := by
  unfold genFun
...
/-- `q = θ₀/θ₂` is a fixed point of the generating function, restated through
`genFun`. -/
lemma genFun_hairy_fixed (h : IsHairy θ₀ θ₁ θ₂) :
    genFun 2 (hairyLaw θ₀ θ₁ θ₂) (θ₀ / θ₂) = θ₀ / θ₂ := by
  rw [genFun_hairy]
  exact h.extinction_fixed_point
