...
* `pairLab`, `pairLab'`: **the two label maps**, with `labMass_pairLab` and
  `labMass_pairLab'` **`thm:shape-coupling`\labelcref{it:shape-coupling-law}**, their
  common law `π`, and `hairyMeasure_pairLab_prod`, `hairyMeasure_pairLab'_prod` its product
  form.
* `markedQI_of_compat_pairLab`: **`thm:shape-coupling`\labelcref{it:shape-coupling-qi}**,
  matched labels give comparable shapes at the scale `19683D⁷`.
* `etaG_le_of_proj`, `gdeg_shapeNet_le_gdeg_pairNet`, `etaG_pairNet_le`:
  **`thm:shape-coupling`\labelcref{it:shape-coupling-eta}**, the ball of a pair contains the
  ...
  `def:shape-net`.
* `hairy_rate_shape_tree_cross`, `hairy_ae_shape_tree_cross`: **`thm:hairy` across two
  laws**, `eq:hairy-rate` at the scale `8·19683²D¹⁴` and the almost sure statement, whose
  only hypothesis besides the two laws is a coupling at every large scale.
* `diagCoupling`, `hairy_ae_shape_tree_diag`: the diagonal coupling of one law with itself,
  and `thm:hairy` for one law read through the pairs.
-/
...
/-- The index of a shape is a left inverse of the enumeration. -/
lemma shapeIdx_shapeEnum (i : ℕ) : shapeIdx (shapeEnum i) = i :=
  shapeEnum_injective (shapeEnum_shapeIdx (shapeEnum i))
...
`thm:shape-mass`\labelcref{it:shape-mass-point} bounds the mass below. -/
...
/-- **`thm:shape-coupling`\labelcref{it:shape-coupling-law} for the first law**: the label
...
/-- **`thm:shape-coupling`\labelcref{it:shape-coupling-law} for the second law**: the same
...
/-- **`thm:shape-coupling`\labelcref{it:shape-coupling-eta}**: the potential of the law on
...
/-- **`thm:hairy` across two laws**: two independent samples of two supercritical laws on
`{0,1,2}` with `θ₀,θ₀'>0` are almost surely quasi-isometric, over the couplings of
`thm:shape-coupling`.  The scales of `def:shape-net` at which the potential is arbitrarily
small are supplied by `eq:shape-eta` for the first law. -/
theorem hairy_ae_shape_tree_cross (θ θ' : Offspring 2) (hq : θ.extinction < 1)
    (hq0 : 0 < θ.extinction) (h2 : 0 < θ 2) (hq' : θ'.extinction < 1)
    (hq0' : 0 < θ'.extinction) (h2' : 0 < θ' 2)
    (hcoup : ∀ D : ℕ, 30 ≤ D → ∃ π : PMF (Shape × Shape),
...
/-! ### The diagonal coupling of one law -/

/-- **The diagonal coupling**: the law of a shape read on the diagonal of the pairs, the
coupling of a shape law with itself. -/
noncomputable def diagCoupling (q : PMF Shape) : PMF (Shape × Shape) :=
  ⟨fun p => if p.1 = p.2 then q p.1 else 0, by
    have hsum : ∑' p : Shape × Shape, (if p.1 = p.2 then q p.1 else 0) = 1 := by
      have hprod : ∑' p : Shape × Shape, (if p.1 = p.2 then q p.1 else 0)
          = ∑' a : Shape, ∑' b : Shape, (if (a, b).1 = (a, b).2 then q (a, b).1 else 0) :=
        ENNReal.tsum_prod'
      rw [hprod]
      calc ∑' a : Shape, ∑' b : Shape, (if a = b then q a else 0)
          = ∑' a : Shape, q a := by
...
@[simp] lemma diagCoupling_apply (q : PMF Shape) (p : Shape × Shape) :
    diagCoupling q p = if p.1 = p.2 then q p.1 else 0 := rfl

lemma margFst_diagCoupling (q : PMF Shape) (a : Shape) : margFst (diagCoupling q) a = q a := by
  rw [margFst]
  simp

lemma margSnd_diagCoupling (q : PMF Shape) (b : Shape) : margSnd (diagCoupling q) b = q b := by
  rw [margSnd]
  simp

/-- The diagonal coupling is a coupling of a shape law with itself: a pair it charges has
equal components. -/
theorem isShapeCoupling_diagCoupling {Dq : ℝ} (hD : 1 ≤ Dq) (q : PMF Shape) :
    IsShapeCoupling Dq q q (diagCoupling q) where
  marg₁ := margFst_diagCoupling q
  marg₂ := margSnd_diagCoupling q
  qi := by
    intro p hp
    have hpp : p.1 = p.2 := by
      by_contra hne
      exact hp (by rw [diagCoupling_apply, if_neg hne])
    rw [hpp]
    exact markedQI_id (one_le_coupScale hD) _
  qi' := by
    intro p hp
    have hpp : p.1 = p.2 := by
      by_contra hne
...
/-- **`thm:hairy` for one law over the diagonal coupling**: the two-law statement applied
to one law, where the coupling is the diagonal.  This is `hairy_ae_shape_tree` read through
the pairs. -/
theorem hairy_ae_shape_tree_diag (θ : Offspring 2) (hq : θ.extinction < 1)
    (hq0 : 0 < θ.extinction) (h2 : 0 < θ 2) :
    twoHairyMeasure θ θ
      {ω | ¬ ∃ (L : ℝ) (F : {v : Amb // v ∈ sample ω.1.1} → {v : Amb // v ∈ sample ω.2.1}),
        IsSampleQI L F} = 0 :=
  hairy_ae_shape_tree_cross θ θ hq hq0 h2 hq hq0 h2 fun D hD => by
    have hDR : (1 : ℝ) ≤ (D : ℝ) := by exact_mod_cast (by omega : 1 ≤ D)
    exact ⟨diagCoupling (shapePMF θ hq hq0 h2),
      isShapeCoupling_diagCoupling hDR (shapePMF θ hq hq0 h2)⟩
