...
* `hasThreeRaysN_of_splits`: **`thm:regime-obstructions-general`
  \labelcref{it:gen-obstr-rays}, the deterministic form**: three rays out of a split
  below a split.
* `exists_isLine_throughN`, `exists_divergeFin`, `nearLineN_of_split`:
  **`thm:regime-obstructions-general`\labelcref{it:gen-obstr-lines}, the deterministic
...
* `exists_componentCompl_coneN`, `exists_isHair_coneN`, `unboundedHairsN_of_deep_cones`:
  **`thm:regime-obstructions-general`\labelcref{it:gen-obstr-hair}, the deterministic
  form**: a finite cone above a child is a hair.
* `ball_subset_neck`, `hasThinBallsN_of_necks`: **`thm:regime-obstructions-general`
  \labelcref{it:gen-obstr-thin}, the deterministic form**: the midpoint of a neck of
  length `2ℓ` has a thin ball.
* `iterFail`, `measure_iterFail_le`, `ae_exists_iterate_mem`: the iteration scheme.
* `mem_skeleton_ambSub_iff`, `gSampleGraph`, `exists_bushAt_eq_ambSub`,
  `exists_two_skeleton_children`, `firstSurvivor`, `neckVertex`, `neckVertex_spec`,
...
* `ae_gArity_ge_two`, `ae_gArity_gSplitBush_ge_two`, `hasThreeRaysN_sample`,
  ...
  **`thm:regime-obstructions-general`\labelcref{it:gen-obstr-rays}**.
* `ae_forall_pos_of_zero`, `nearLineN_sample`, `nearLine_gSample_ae`:
  **`thm:regime-obstructions-general`\labelcref{it:gen-obstr-lines}**.
* `ae_exists_neck`, `hasThinBallsN_sample`, `thinBalls_gSample_ae`:
  **`thm:regime-obstructions-general`\labelcref{it:gen-obstr-thin}**.
* `deepEvent`, `bushMeasure_deepEvent_pos`, `hairEvent`, `hairMass_ne_zero`,
  ...
  **`thm:regime-obstructions-general`\labelcref{it:gen-obstr-hair}**.
-/
...
/-- **`thm:regime-obstructions-general`\labelcref{it:gen-obstr-rays}**, in the form
...
/-- **`thm:regime-obstructions-general`\labelcref{it:gen-obstr-lines}**, in the form
...
/-- **`thm:regime-obstructions-general`\labelcref{it:gen-obstr-hair}**, in the form
...
/-- **`thm:regime-obstructions-general`\labelcref{it:gen-obstr-thin}**, in the form
...
/-- **`thm:regime-obstructions-general`\labelcref{it:gen-obstr-rays}, the
...
/-- **`thm:regime-obstructions-general`\labelcref{it:gen-obstr-lines}, the
...
/-- **`thm:regime-obstructions-general`\labelcref{it:gen-obstr-thin}, the
...
/-- **The `m`-th surviving subtree is the residual field at a skeleton child.** -/
lemma exists_bushAt_eq_ambSub {m : ℕ} (hm : m < skeletonDegree d) :
    ∃ i : Fin N, [i] ∈ skeleton d ∧ bushAt d m = ambSub d [i] :=
  ⟨(survivors d).orderEmbOfFin rfl ⟨m, hm⟩,
    BranchingProcess.mem_survivors_iff_mem_skeleton.mp
      (BranchingProcess.orderEmbOfFin_mem_survivors hm),
    BranchingProcess.bushAt_of_lt hm⟩
...
/-- **`thm:regime-obstructions-general`\labelcref{it:gen-obstr-rays}, read in the
...
/-- **`thm:regime-obstructions-general`\labelcref{it:gen-obstr-rays}**: almost surely
...
/-- **`thm:regime-obstructions-general`\labelcref{it:gen-obstr-rays}** under the
...
/-- **`thm:regime-obstructions-general`\labelcref{it:gen-obstr-lines}, read in the
...
/-- **`thm:regime-obstructions-general`\labelcref{it:gen-obstr-lines}**: at `θ₀ = 0`,
...
/-- **`thm:regime-obstructions-general`\labelcref{it:gen-obstr-thin}, read in the
...
/-- **`thm:regime-obstructions-general`\labelcref{it:gen-obstr-thin}**: at
...
/-- **`thm:regime-obstructions-general`\labelcref{it:gen-obstr-hair}, read in the
...
/-- **`thm:regime-obstructions-general`\labelcref{it:gen-obstr-hair}**: at `θ₀ > 0`,
