...
* `isLine_of`, `downWord`, `upWord`, `exists_isLine_through`, `NearLine`,
  ...
  **`thm:regime-obstructions`\labelcref{it:obstr-lines}**.  A tree in which
...
* `hasThreeRays_of`, `childRay`, `upRay`, `hasThreeRays_sampleWord`,
  `raysInHairyRegime`: **`thm:regime-obstructions`\labelcref{it:obstr-rays} in
...
* `scaleCondition_holds`: the largeness conditions of `thm:cross-law` at scales
  driving its rate to zero.
...
`thm:regime-obstructions`\labelcref{it:obstr-hair} is `hairsInHairyRegime` of
...
`thm:regime-obstructions`\labelcref{it:obstr-lines}: a tree whose vertices all
...
/-- **`thm:regime-obstructions`\labelcref{it:obstr-lines}, the deterministic
...
/-- **`thm:regime-obstructions`\labelcref{it:obstr-lines}**, in the form
...
/-- **`thm:regime-obstructions`\labelcref{it:obstr-lines} in the chain
...
/-- **`thm:regime-obstructions`\labelcref{it:obstr-rays}**, in the form
...
lemma SampleLaw.isTree (L : SampleLaw) (ω : L.Ω) : (L.graph ω).IsTree :=
  wordGraph_isTree (L.prefixClosed ω) ⟨⟨[], L.root ω⟩⟩
...
/-- **`thm:regime-obstructions`\labelcref{it:obstr-rays} in class (B)**, the
...
/-- The largeness conditions `eq:d0-conditions` of `thm:cross-law`, at scales
driving the rate of `eq:rate-cross` to zero. -/
def ScaleCondition : Prop :=
  ∀ t t' : ℝ, 0 < t → t < 1 → 0 < t' → t' < 1 → ∀ ε : ℝ, 0 < ε → ∃ D : ℕ, 5 ≤ D ∧
    1 ≤ cgamma (1 - t) (1 - t') * ((D : ℝ) - 1) ∧
    (1 - t) ^ (D - 1) ≤ 1 / 10 ∧ (1 - t) ^ (D ^ 2 - D) ≤ 1 / 2 ∧
    36 * (1 - t) ^ (5 * D - 2) ≤ 1 ∧ 16 * qBound (1 - t) D ≤ 1 / 10000 ∧
    256 * qBound (1 - t) D < ε

/-- **The largeness conditions hold**, from `exists_scale_cross` of
`ChainClasses/CrossLaw.lean`, where the analysis now lives. -/
theorem scaleCondition_holds : ScaleCondition :=
  fun _ _ ht ht1 ht' ht1' _ hε => exists_scale_cross ht ht1 ht' ht1' hε
...
`thm:regime-obstructions`\labelcref{it:obstr-rays}. -/
...
`thm:regime-obstructions`\labelcref{it:obstr-rays} in class (B). On the event
