...
lemma restrictLab_branch_zero {V : Type} (s : V) (p : FullLab V 0 × FullLab V 0) :
    restrictLab 0 (GraphMarkovMatching.branch s p) = leaf s := rfl
...
lemma subL_branch {n : ℕ} (s : V) (p : FullLab V n × FullLab V n) :
    subL (GraphMarkovMatching.branch s p) = p.1 := rfl

lemma subR_branch {n : ℕ} (s : V) (p : FullLab V n × FullLab V n) :
    subR (GraphMarkovMatching.branch s p) = p.2 := rfl
...
lemma recMass_nil (μ ν : PMF ℕ) : recMass μ ν ([] : List (Rec N')) = 1 := rfl
...
lemma gChild_prefix_append (u : GWord N') {j : ℕ} (h : j < N') (i : Fin N') :
    gChild u j <+: u ++ [i] ↔ (i : ℕ) = j := by
  rw [gChild_of_lt u h]
  constructor
  · intro e
    have := (e.eq_of_length (by simp)).symm
    have := (List.append_inj' this rfl).2
    simp only [List.cons.injEq, and_true] at this
    rw [this]
  · intro e
    have : i = ⟨j, h⟩ := Fin.ext e
    rw [this]
...
/-- The event that the fields realise a list of records is measurable. -/
lemma measurableSet_levent (D : ℝ) {θ' : Offspring J'} (π : GCouplings θ')
    (L : List (Rec N')) :
    MeasurableSet {ω : (GWord N' → ℕ) × (GWord N' → ℝ) |
      Levent L (gLab D π ω) (gArityAt ω.1)} :=
  measurableSet_levent_of (fun w a => measurableSet_gLab_fibre D π w a)
    (fun w k => measurable_fst (fibreMeasurableG_gArityAt w k)) L
