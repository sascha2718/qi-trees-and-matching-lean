...
* `sampleLetter`, `transSample`: the letter map of a sample and the translation it
  carries.
* `Shape.isAddr_neckAddr_take`, `Shape.isAddr_neckAddr_concat_none`,
  `Shape.isAddr_neckAddr_concat_some`, `Shape.isAddr_bush` and `Shape.isAddr_cases`: the
...
/-- **`def:shape`**: the neck vertices are addresses of the realisation. -/
lemma Shape.isAddr_neckAddr_take : ∀ (l : List (Option Tri)) (i : ℕ),
    (Shape.realiseAux l).IsAddr (Shape.neckAddr (l.take i))
  | l, 0 => by simp
  | [], i + 1 => by simp
  | o :: l, i + 1 => by
      have ih := Shape.isAddr_neckAddr_take l i
      cases o with
