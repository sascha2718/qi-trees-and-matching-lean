...
The potential estimate is the split of `thm:shape-coupling`\labelcref{it:shape-coupling-eta}
...
* `markedQI_of_compat_pairLabS`:
  **`thm:shape-coupling`\labelcref{it:shape-coupling-qi}**, matched labels give a
...
* `exists_isShapeCoupling_etaS`:
  **`thm:shape-coupling`\labelcref{it:shape-coupling-eta}**, the coupling with
...
/-! ### `thm:shape-coupling`\labelcref{it:shape-coupling-qi} at the edge scale -/
...
/-- **`thm:shape-coupling`\labelcref{it:shape-coupling-eta}**: past a threshold, the two
