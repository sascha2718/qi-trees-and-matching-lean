...
**`thm:shape-mass`\labelcref{it:shape-mass-tail}** at the shape law itself.
...
* `shapeStatMassE`, `shapeStatMass`, `survivalMeasure_shape_stat`,
  `tsum_pow_shapeStatMassE`, `summable_shapeStatMass_mgf`: **the law of a size statistic
  of the shape**, read on a sample, and its moment as the real series `chernoff_tail`
  consumes.
* `shapeSizeMass`, `exists_shape_stat_tail`, `exists_shape_size_tail`:
  **`thm:shape-mass`\labelcref{it:shape-mass-tail}**, the exponential tail of the size of
  a shape.
* `fixSizeMass`, `exists_fix_size_tail`: the same for the support fix of
  `thm:shape-shrink`\labelcref{it:shape-fix}, whose size at most doubles, so the rate
  halves.
-/
...
/-- **The law of a statistic read on a sample**: conditioned on survival the statistic of
the shape at any copy has the law `shapeStatMassE`. -/
theorem survivalMeasure_shape_stat (hq : θ.extinction < 1) (hq0 : 0 < θ.extinction)
    (h2 : 0 < θ 2) (f : Shape → ℕ) (w : Word) (n : ℕ) :
    survivalMeasure (N := 2) θ {c : Amb → ℕ | f (shapeAt c w) = n} = shapeStatMassE θ f n := by
  have hcover : {c : Amb → ℕ | f (shapeAt c w) = n}
      = ⋃ σ : ↥{σ : Shape | f σ = n}, {c : Amb → ℕ | shapeAt c w = σ.1} := by
    ext c
    simp only [Set.mem_setOf_eq, Set.mem_iUnion]
    constructor
    · intro h
      exact ⟨⟨shapeAt c w, h⟩, rfl⟩
    · rintro ⟨σ, hσ⟩
      have hc : shapeAt c w = σ.1 := hσ
      rw [hc]
      exact σ.2
  have hdisj : Pairwise (Function.onFun Disjoint
      fun σ : ↥{σ : Shape | f σ = n} ↦ {c : Amb → ℕ | shapeAt c w = σ.1}) := by
    intro σ τ hst
    refine Set.disjoint_left.mpr fun c hc hc' ↦ hst (Subtype.ext ?_)
    exact (hc : shapeAt c w = σ.1).symm.trans hc'
  calc survivalMeasure (N := 2) θ {c : Amb → ℕ | f (shapeAt c w) = n}
      = ∑' σ : ↥{σ : Shape | f σ = n},
          survivalMeasure (N := 2) θ {c : Amb → ℕ | shapeAt c w = σ.1} := by
        rw [hcover]
        exact measure_iUnion hdisj fun σ ↦ measurableSet_shapeAt_eq w σ.1
    _ = ∑' σ : ↥{σ : Shape | f σ = n}, shapeMass θ σ.1 :=
        tsum_congr fun σ ↦ survivalMeasure_shapeAt θ hq hq0 h2 w σ.1
    _ = ∑' σ : Shape, Set.indicator {σ : Shape | f σ = n} (shapeMass θ) σ :=
        tsum_subtype _ _
    _ = shapeStatMassE θ f n := by
        rw [shapeStatMassE]
        refine tsum_congr fun σ ↦ ?_
        by_cases h : f σ = n <;> simp [h]
...
/-- **`thm:shape-mass`\labelcref{it:shape-mass-tail} for a statistic of the shape**: a
...
/-- **`thm:shape-mass`\labelcref{it:shape-mass-tail}**: the size of a shape has an
exponential tail. -/
...
/-- **`thm:shape-mass`\labelcref{it:shape-mass-tail} for the support fix**: the fix at
