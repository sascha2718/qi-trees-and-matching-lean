/-
`thm:relabel`\labelcref{it:relabel-eta} and
`thm:cross-relabel`\labelcref{it:cross-relabel-eta} of `matching_classes_general.tex`:
...
* `tsum_ite_map`, `gClassPMF`, `gClassPMF_eq_tsum`, `gMixPMF_le_gClassPMF`,
  `rE_gClassPMF`, `gdeg_gClassPMF`: **the class law `μ_D`**, the mass of a class as the
  sum of the mixture over its fibre, and the ball mass of a class read off the shapes.
...
* `exists_etaG_gNetGraph_le`: **`thm:relabel`\labelcref{it:relabel-eta}**, the bound
...
/-- A shape carries its mass into its class. -/
lemma gMixPMF_le_gClassPMF (θ : Offspring J) (hJN : J ≤ N) (hq : θ.extinction < 1)
    (hq0 : 0 < θ.extinction) (hs1 : θ.skeletonWeight 1 < 1) (Dq : ℝ) (σ : GShape) :
    gMixPMF θ hJN hq hq0 hs1 σ ≤ gClassPMF θ hJN hq hq0 hs1 Dq (gNetLab Dq σ) := by
  rw [gClassPMF_eq_tsum]
  calc gMixPMF θ hJN hq hq0 hs1 σ
      = (if gNetLab Dq σ = gNetLab Dq σ then gMixPMF θ hJN hq hq0 hs1 σ else 0) :=
        (if_pos rfl).symm
    _ ≤ ∑' τ : GShape, if gNetLab Dq τ = gNetLab Dq σ then gMixPMF θ hJN hq hq0 hs1 τ else 0 :=
        ENNReal.le_tsum σ
...
/-- **The ball mass of a class read off the shapes.** -/
lemma gdeg_gClassPMF (θ : Offspring J) (hJN : J ≤ N) (hq : θ.extinction < 1)
    (hq0 : 0 < θ.extinction) (hs1 : θ.skeletonWeight 1 < 1) (Dq : ℝ) (v : ℕ) :
    gdeg (gClassPMF θ hJN hq hq0 hs1 Dq) (gNetGraph Dq) v
      = (∑' σ : GShape,
          if compat (gNetGraph Dq) v (gNetLab Dq σ) then gMixPMF θ hJN hq hq0 hs1 σ
            else 0).toReal := by
  rw [gdeg, rE_gClassPMF]
...
/-- **`thm:relabel`\labelcref{it:relabel-eta}, the collapse**: a shape of size at most
...
/-- **`thm:relabel`\labelcref{it:relabel-eta}, the mass of `v₀`**: once the mass above
...
/-- **`thm:relabel`\labelcref{it:relabel-eta}, the mass of `v₀`**: past a threshold the
...
/-! ### `thm:relabel`\labelcref{it:relabel-eta} -/

/-- **`thm:relabel`\labelcref{it:relabel-eta}**, in the exponential form: the potential
...
statement is `thm:cross-relabel`\labelcref{it:cross-relabel-eta}, the law `μ_D` and the
...
/-- **`thm:relabel`\labelcref{it:relabel-eta}, at an arbitrary threshold**: the potential
...
/-- **`thm:relabel`\labelcref{it:relabel-eta}, the potential condition of
...
`thm:cross-relabel`\labelcref{it:cross-relabel-eta}. -/
