...
`thm:chain-cross` (`it:chain-cross-product`) of `trichotomy.tex`: the labelled
...
* `crossLab_pattern`, `crossLab_marginal`:
  **`thm:chain-cross` (`it:chain-cross-product`)**, the coupled labels of the
...
`u`-measure of class `k` at the length `m`, sum to `p^{(D)}_k/(1-b)`. -/
...
/-- **`thm:chain-cross` (`it:chain-cross-product`), the coupled side**: over the
