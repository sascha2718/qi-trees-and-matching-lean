...
`thm:shape-shrink`\labelcref{it:shape-shrink} at general bounded support, the map the
...
* `gShrinkScale`, `gShrinkScale_le`: the scale `⌊√(D/216J²)⌋` and the bound
  `216 J² s² ≤ D` it gives.
* `markedQI_gShrink_of_le`: **`thm:shape-shrink`\labelcref{it:shape-shrink} at general
...
/-- **`thm:shape-shrink`\labelcref{it:shape-shrink}, the quasi-isometry at general
...
/-- **`thm:shape-shrink`\labelcref{it:shape-shrink}, the size at general arity**: the
...
/-- **`thm:shape-shrink`\labelcref{it:shape-shrink}, the support at general arity**:
...
/-- **`thm:shape-shrink`\labelcref{it:shape-shrink} at general arity**: for every
