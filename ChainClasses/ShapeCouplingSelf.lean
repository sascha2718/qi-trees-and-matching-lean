...
`thm:shape-shrink`\labelcref{it:shape-fix}, which is the identity on the support.  For one
law the coupling of `thm:shape-coupling`\labelcref{it:shape-coupling-law} is the diagonal,
...
* `markedQI_of_compat_shapeLabel`: **`thm:shape-coupling`\labelcref{it:shape-coupling-qi}**,
...
* `hairy_rate_shape_self`, `hairy_rate_shape_tree_self`, `hairy_ae_shape_tree_self`:
  **`thm:hairy` for one law**, `eq:hairy-rate` for the assemblies and for the samples at
  the scale `8·729²D⁸`, and the almost sure statement, over the potential bound of
  `thm:shape-coupling`\labelcref{it:shape-coupling-eta}.
-/
...
/-- **`eq:hairy-rate` for the assemblies, one law**: with the shapes as labels the failure
probability of the quasi-isometry between the two assemblies is bounded by the potential
of the label law. -/
theorem hairy_rate_shape_self (θ : Offspring 2) (hq : θ.extinction < 1)
    ...
    twoHairyMeasure θ θ
        {ω | ¬ ∃ f : Assembly (shapeAt ω.1.1) → Assembly (shapeAt ω.2.1),
          IsQIMap (8 * (729 * Dq ^ 4) ^ 2) f}
      ≤ 16 * etaG (shapePMF θ hq hq0 h2) (shapeNet Dq) := by
  have hD0 : (0 : ℝ) ≤ Dq := le_trans zero_le_one hD
  have h4 : (1 : ℝ) ≤ Dq ^ 4 := one_le_pow₀ hD
  exact hairy_rate θ θ hq hq (shapePMF θ hq hq0 h2) (shapeNet Dq) (by nlinarith)
