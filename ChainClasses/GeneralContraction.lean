...
lemma isAddr_of_isAddr_append : ∀ (t : RTree) (u v : List ℕ), IsAddr t (u ++ v) → IsAddr t u := by
  intro t u
  induction u generalizing t with
  | nil => intro _ _; exact isAddr_nil t
  | cons i u ih =>
      intro v h
      cases t with
      | node cs =>
          rw [List.cons_append, isAddr_cons] at h
          obtain ⟨hi, h⟩ := h
          exact isAddr_cons.mpr ⟨hi, ih _ v h⟩
...
lemma isPartRoot_iff (T : RTree) (w : List ℕ) : IsPartRoot s T w ↔ part s T w = w := Iff.rfl

/-- The part of any address is a part root. -/
lemma isPartRoot_part (T : RTree) (w : List ℕ) : IsPartRoot s T (part s T w) := part_idem T w
...
/-- At scale one the mark is the index of the marked address. -/
lemma markOf_one (T : RTree) (e : List ℕ) : markOf 1 T e = (partList 1 T).idxOf e := by
  rw [markOf, part_one]
