/-
`thm:relabel`\labelcref{it:relabel-law} of `matching_classes_general.tex`: the
...
* `condPMF`, `condDraw`, `volume_condDraw_fibre`, `margFstT_mul_volume_condDraw_fibre`,
  `volume_condDraw_mem`: **the conditional draw** `ℓ_k(σ, u)` from a coupling given its
...
/-- The conditional draw lands in a set with the conditional mass of the set. -/
lemma volume_condDraw_mem (π : PMF (T × T)) {σ : T} (h : margFstT π σ ≠ 0) (B : Set T) :
    volume {u ∈ Set.Ico (0 : ℝ) 1 | condDraw π σ u ∈ B}
      = ∑' τ : B, π (σ, τ) / margFstT π σ := by
  have hcover : {u ∈ Set.Ico (0 : ℝ) 1 | condDraw π σ u ∈ B}
      = ⋃ τ : B, {u ∈ Set.Ico (0 : ℝ) 1 | condDraw π σ u = τ} := by
    ext u
    ...
    · rintro ⟨τ, hτ, hu, rfl⟩
      exact ⟨hu, hτ⟩
  have hdisj : Pairwise (Function.onFun Disjoint fun τ : B ↦
      {u ∈ Set.Ico (0 : ℝ) 1 | condDraw π σ u = τ}) := by
    intro τ τ' hne
    refine Set.disjoint_left.mpr fun u hu hu' ↦ hne (Subtype.ext ?_)
    rw [← hu.2, ← hu'.2]
  have hmeas : ∀ τ : B, MeasurableSet {u ∈ Set.Ico (0 : ℝ) 1 | condDraw π σ u = τ} :=
    fun τ ↦ measurableSet_Ico.inter (measurableSet_condDraw_fibre π σ τ)
  rw [hcover, measure_iUnion hdisj hmeas]
  exact tsum_congr fun τ ↦ volume_condDraw_fibre π h τ
