...
* `Tri.addrSet`, `Tri.mem_addrSet`, `Tri.card_addrSet`: the vertices of a finite tree and
  their number.
* `Tri.count`, `Tri.mem_sample_count`, `Tri.ncard_sample_count`, `Tri.not_survives_count`:
...
* `bushPointBound`: **the bush input of
  `thm:shape-mass`\labelcref{it:shape-mass-point}**, the hypothesis `BushPointBound`
  discharged.
...
* `ofReal_pow_size_le_shapeMass_of_weights`, `shape_mass_point_shapeMass_of_weights` and
  `shape_mass_point_survivalMeasure_of_weights`:
  **`thm:shape-mass`\labelcref{it:shape-mass-point}** at the shape law and at a sample,
  over the offspring weights alone.
-/
...
lemma Tri.not_survives_count (t : Tri) : ¬ Survives t.count :=
  Set.not_infinite.mpr t.finite_sample_count
...
/-- **`thm:shape-mass`\labelcref{it:shape-mass-point}, the bush input**: a bush of `n`
...
/-- **`thm:harris`\labelcref{it:harris-bushes}, the arithmetic**: the conjugate law of a
...
/-- The size of a bush read off the field and read off the bush agree almost surely. -/
lemma bushMeasure_ncard_eq_size (θ : Offspring 2) (n : ℕ) :
    bushMeasure (N := 2) θ {c : Amb → ℕ | (sample c : Set Amb).ncard = n}
      = bushMeasure (N := 2) θ {d : Amb → ℕ | (bushTri d).size = n} := by
  ...
  show ((sample c : Set Amb).ncard = n) = ((bushTri c).size = n)
...
/-- **`thm:shape-mass`\labelcref{it:shape-mass-tail} for the bushes**: the size of a bush
...
/-- **`thm:shape-mass`\labelcref{it:shape-mass-point}** at the shape law, over the
...
`C = 2 log p⁻¹`, over the offspring weights alone. -/
theorem shape_mass_point_shapeMass_of_weights (θ : Offspring 2) (hq : θ.extinction < 1)
    {p : ℝ} (hp0 : 0 < p) (hp1 : p ≤ 1) (h0 : p ≤ θ 0) (h1 : p ≤ θ 1) (h2 : p ≤ θ 2)
    (hk : p ≤ θ.skeletonWeight 2) (σ : Shape) :
    ENNReal.ofReal (Real.exp (-(2 * Real.log p⁻¹) * (σ.size : ℝ))) ≤ shapeMass θ σ := by
  have hd : p ≤ θ 2 * 2 * θ.extinction := by
    rw [decoration_weight_eq θ hq]
    linarith [θ.nonneg 0]
  exact shape_mass_point_shapeMass hp0 hp1 h1 hd hk (bushPointBound θ hp0.le h0 h1 h2) σ

/-- **`thm:shape-mass`\labelcref{it:shape-mass-point}** read on a sample, over the
offspring weights alone: conditioned on survival, the shape at any copy takes a prescribed
value with probability at least `e^{-C|σ|}`. -/
theorem shape_mass_point_survivalMeasure_of_weights (θ : Offspring 2) (hq : θ.extinction < 1)
    (hq0 : 0 < θ.extinction) {p : ℝ} (hp0 : 0 < p) (hp1 : p ≤ 1) (h0 : p ≤ θ 0)
    (h1 : p ≤ θ 1) (h2 : p ≤ θ 2) (hk : p ≤ θ.skeletonWeight 2) (w : Word) (σ : Shape) :
    ENNReal.ofReal (Real.exp (-(2 * Real.log p⁻¹) * (σ.size : ℝ)))
      ≤ BranchingProcess.survivalMeasure (N := 2) θ {c : Amb → ℕ | shapeAt c w = σ} := by
  have hd : p ≤ θ 2 * 2 * θ.extinction := by
    rw [decoration_weight_eq θ hq]
    linarith [θ.nonneg 0]
  have hθ2 : 0 < θ 2 := lt_of_lt_of_le hp0 h2
  exact shape_mass_point_survivalMeasure hq hq0 hθ2 hp0 hp1 h1 hd hk
    (bushPointBound θ hp0.le h0 h1 h2) w σ

/-- The law of the size of a bush, read on the bush itself. -/
lemma bushSizeMass_eq_bushTri (θ : Offspring 2) (n : ℕ) :
    bushSizeMass θ n
      = (bushMeasure (N := 2) θ {d : Amb → ℕ | (bushTri d).size = n}).toReal := by
  rw [bushSizeMass, bushMeasure_ncard_eq_size]
