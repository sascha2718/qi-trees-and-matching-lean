/-
`thm:matched-presentation`\labelcref{it:matched-product} of
...
`matchedShift`, which is clause \labelcref{it:matched-product}, and the law-level
...
* `matched_iid`: **`thm:matched-presentation`\labelcref{it:matched-product}**:
  ...
  `matchedShift`.
* `presentedShift`, `presentedShift_eq`, `presentedShift_sum_eq_one`,
  `presentedShift_pos_iff`, `presentedShift_floor`, `presentedShift_bulk_close`,
  `presentedShift_window`: the presented shift law of the blob presentation on
  the constructed space and the clauses
  \labelcref{it:matched-support,it:matched-mass,it:matched-bulk} for it.
-/
...
/-- **`thm:matched-presentation`\labelcref{it:matched-product}**: over the sample
...
/-- **The presented shift law is a law** on the shifts up to `L + MA`. -/
theorem presentedShift_sum_eq_one :
    ∑ j ∈ Finset.range (L + MA + 1), presentedShift θ R L T hδ0 hδ1 hP' hP'1 j = 1 := by
  rw [Finset.sum_congr rfl fun j _ ↦
    presentedShift_eq θ hq hs1 R hA1 hAsupp hMA hB0 hMB hP' hP'1 hδ0 hδ1 hF hF' hT j]
  exact matchedShift_sum_eq_one hMA hMB hA1 hB0 (sum_reducedWeight_shift θ hq hs1 hA1 hAsupp)
    hP'1 hF hF'

/-- **`thm:matched-presentation`\labelcref{it:matched-support}** on the
...
/-- **`thm:matched-presentation`\labelcref{it:matched-mass}** on the constructed
...
/-- **`thm:matched-presentation`\labelcref{it:matched-bulk}, the window mass** on
...
/-- **`thm:matched-presentation`\labelcref{it:matched-bulk}, the bulk agreement**
