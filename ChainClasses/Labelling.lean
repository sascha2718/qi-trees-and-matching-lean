...
* `iotaL`: the projection `ι` of an abstract labelling `lam : 𝔹 → ℕ`, running
  the recursion `eq:phi-def` on `lam`; `iota_eq_iotaL` identifies it with the
  `iota` of `ChainClasses.Encoding` when `lam` is the label field `lab`.
...
/-! ### The projection of an abstract labelling -/
...
/-- `ι` reads the labelling only on strict prefixes. -/
lemma iotaL_congr {lam lam' : Word → ℕ} {w : Word}
    (h : ∀ u, u <+: w → u ≠ w → lam u = lam' u) :
    iotaL lam w = iotaL lam' w := by
  induction w using List.reverseRecOn with
  | nil => rfl
  | append_singleton w j ih =>
      have hw : lam w = lam' w := by
        refine h w (List.prefix_append _ _) fun hc => ?_
        have := congrArg List.length hc
        simp at this
      have hih : iotaL lam w = iotaL lam' w := by
        refine ih fun u hu hne => h u (hu.trans (List.prefix_append _ _)) fun hc => ?_
        rw [hc] at hu
        have := hu.length_le
        simp only [List.length_append, List.length_singleton] at this
        omega
      rw [iotaL_concat, iotaL_concat, hw, hih]

/-! ### The bridge to `ChainClasses.Encoding` -/

variable {χ : Word → Bool}

/-- On a sample tree, `iota` is the projection of the label field: `iota`
