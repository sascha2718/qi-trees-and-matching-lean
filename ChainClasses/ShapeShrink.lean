...
the support fix of `thm:shape-shrink`\labelcref{it:shape-fix}.
...
* `markedQI_shape_fix`, `size_fix_le`, `fix_eq_self`: the three clauses of
  `thm:shape-shrink`\labelcref{it:shape-fix}.
-/
...
/-- **`thm:shape-shrink`\labelcref{it:shape-fix}**: the fix lies in the
support. -/
...
/-- **`thm:shape-shrink`\labelcref{it:shape-fix}**: the fix at most doubles
the size. -/
...
/-- **`thm:shape-shrink`\labelcref{it:shape-fix}**: a shape already in the
...
/-- **`thm:shape-shrink`\labelcref{it:shape-fix}**: the inclusion of a shape
