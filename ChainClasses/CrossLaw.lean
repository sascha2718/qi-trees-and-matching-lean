...
* `chainMeasure_label_map`, `chainMeasure_class_lintegral`,
  ...
  i.i.d. with the class law `q = p^{(D)}` of the first. The joint law over a
...
`u`-measure of the class over the chain length gives the class mass `q_k`. -/
...
second sample at a vertex has the class law `q_k = p^{(D)}_k` of the first. -/
