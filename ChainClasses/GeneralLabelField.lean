/-
`thm:relabel`\labelcref{it:relabel-law}, `thm:product-form` of
`matching_classes_general.tex` and `thm:hairy-cross`\labelcref{it:hairy-cross-product}
...
laws coupled to the one mixture it is `thm:hairy-cross`\labelcref{it:hairy-cross-product}.
...
* `measurableSet_gDraw_fibre`, `measurableSet_gPartner_fibre`, `measurableSet_gLab_fibre`,
  `measurable_gLab`: the fields are measurable on the labelled space.
* `labelMeasure_partner_pattern`: **`thm:relabel`\labelcref{it:relabel-law}**, the
...
* `labelMeasure_label_pattern`: **`thm:product-form` and
  `thm:hairy-cross`\labelcref{it:hairy-cross-product}**, the pairs of a label and an
...
lemma measurable_gLab (D : ℝ) {θ' : Offspring J'} (π : GCouplings θ') (u : GWord N') :
    Measurable fun ω : (GWord N' → ℕ) × (GWord N' → ℝ) ↦ gLab D π ω u :=
  measurable_to_countable' fun a ↦ measurableSet_gLab_fibre D π u a

/-! ### The partners are i.i.d. with the mixture law -/

/-- **`thm:relabel`\labelcref{it:relabel-law}, over the constructed space**: with a
...
/-- **`thm:product-form` and `thm:hairy-cross`\labelcref{it:hairy-cross-product}, over
