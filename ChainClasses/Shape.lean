...
* `Tri.del` and `exists_shrink`: the leaf deletion of `thm:shape-connected`,
  a leaf of a nonempty decoration if there is one and the exit otherwise, and
  the shape of size one less that it leaves; `eq_of_size_le_one` is the base of
  that descent. The deletion map is a `1`-marked quasi-isometry, which the
  metric on realisations would state.
-/
...
/-- Deleting a leaf of a nonempty decoration: the neck length is kept and one
vertex is dropped. -/
lemma exists_decs_del : ∀ l : List (Option Tri), 1 ≤ (l.map Tri.optSize).sum →
    ∃ l' : List (Option Tri), l'.length = l.length ∧
      (l'.map Tri.optSize).sum + 1 = (l.map Tri.optSize).sum
  | [], h => by simp at h
  | none :: bs, h => by
      simp only [List.map_cons, List.sum_cons, Tri.optSize_none, Nat.zero_add] at h
      obtain ⟨l', hlen, hsum⟩ := exists_decs_del bs h
      refine ⟨none :: l', by simp [hlen], ?_⟩
      simp only [List.map_cons, List.sum_cons, Tri.optSize_none, Nat.zero_add]
      exact hsum
  | some b :: bs, _ => by
      have hb := Tri.optSize_del b
      refine ⟨b.del :: bs, by simp, ?_⟩
      simp only [List.map_cons, List.sum_cons, Tri.optSize_some]
      omega
...
theorem exists_shrink (σ : Shape) (hσ : 2 ≤ σ.size) : ∃ τ : Shape, τ.size + 1 = σ.size := by
  by_cases h : 1 ≤ (σ.decs.map Tri.optSize).sum
  · obtain ⟨l', hlen, hsum⟩ := exists_decs_del σ.decs h
    refine ⟨ofList l', ?_⟩
    rw [size_ofList, size_eq' σ, neckLen, hlen, decs_length]
    omega
  · have hzero : (σ.decs.map Tri.optSize).sum = 0 := by omega
    have hsize : σ.size = σ.necks + 1 := by rw [size_eq', hzero, neckLen]
    refine ⟨ofList (List.replicate (σ.necks - 1) none), ?_⟩
    rw [size_ofList, sum_optSize_replicate, List.length_replicate]
    omega
