...
* `RTree.lcrs`, `RTree.lcrsAddr`, `markedQI_lcrs`: **`thm:shape-shrink`
  \labelcref{it:shape-shrink}, the binarisation** at general arity: the first child at
...
lemma realise_gOne : gOne.realise = .node [] := rfl
...
lemma length_lcrsF_eq_zero_iff {cs : List RTree} : (lcrsF cs).length = 0 ↔ cs = [] := by
  cases cs <;> simp
...
/-- **`thm:shape-shrink`\labelcref{it:shape-shrink}, the binarisation at general
...
/-- **`thm:shape-shrink`\labelcref{it:shape-shrink}, the moves after the contraction at
