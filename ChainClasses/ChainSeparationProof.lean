...
import ChainClasses.GeneralTrichotomy
...
/-- A line through the zero letter is a ray vertex. -/
lemma lineW_zero {a : ℕ} (t : ℕ) :
    lineW hN a L 0 (⟨0, hN⟩ : Fin N) t = zw hN (a + L + 1 + t) := by
...
/-- An auxiliary law-level equivalence underlying **`thm:trichotomy`**, with both chain-regime
statements discharged. Almost-sure quasi-isometry holds exactly when the two laws have the same
class invariant. -/
theorem trichotomy (R R' : GRegime) :
  ...
  trichotomy_general chainUniversality chainSeparation R R'
...
coarse regime together with the branching semigroup in the chain regime. Unlike `trichotomy`,
this statement retains the almost-sure negative conclusion when the invariants differ. -/
