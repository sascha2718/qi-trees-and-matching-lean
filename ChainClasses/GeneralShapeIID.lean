...
* `survivalMeasure_gArities`: **the arity field is i.i.d. `ν̃`**,
  `thm:harris-general`\labelcref{it:harris-general-reduced} over reduced-skeleton
  probes.
...
`thm:harris-general`\labelcref{it:harris-general-reduced} probed on the reduced
