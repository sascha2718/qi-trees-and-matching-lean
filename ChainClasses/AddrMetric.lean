...
* `addrDist_of_prefix`, `addrDist_append_append`, `addrDist_cons_cons_self`,
  `addrDist_cons_cons_ne`, `addrDist_dropLast`: the distance to an ancestor, inside a
  subtree, across a divergence, and to the parent.
...
/-- An address and its parent are at distance one. -/
lemma addrDist_dropLast {x : List ℕ} (hx : x ≠ []) : addrDist x x.dropLast = 1 := by
  rw [addrDist_comm, addrDist_of_prefix (List.dropLast_prefix x), List.length_dropLast]
