...
* Clause (ii) is `coupling_law`: `geomPMF` packages the second law
  ...
  of the proof, and `coupling_law` is the identity `ℙ(ℓ'(λ', U) = k) = q_k`.
...
law and the uniform variable `U` on `[0,1)`, the coupled class has the law `q`
of `thm:chain-classes`, `ℙ(ℓ'(λ', U) = k) = q_k`. -/
