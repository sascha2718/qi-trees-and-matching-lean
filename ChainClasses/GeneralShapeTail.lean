...
* `gBushMoment`, `tsum_bushMassR_mul_pow`, `tsum_bushMassR_le_one`,
  `exists_bush_moment_near_one`: **the bush moment**, read as the exponential moment of
...
/-- The bush masses sum to at most one. -/
lemma tsum_bushMassR_le_one (θ : Offspring J) (hJN : J ≤ N) (hq0 : 0 < θ.extinction) :
    ∑' t : RTree, bushMassR (N := N) θ t ≤ 1 := by
  have _ := BranchingProcess.isProbabilityMeasure_bushMeasure θ hJN hq0
  have h : ∑' t : RTree, bushMassR (N := N) θ t = gBushMoment (N := N) θ 1 := by
    rw [gBushMoment]
    simp only [one_pow, mul_one]
  rw [h, tsum_bushMassR_mul_pow θ hJN]
  simp
