...
* `gShapeIdx`, `gNetGraph`, `gNetLab`: the label graph `G_D` on the classes and the
  class map.
* `crossRelabel_qi`: **`thm:cross-relabel`\labelcref{it:cross-relabel-qi}**, the
...
/-- **`thm:cross-relabel`\labelcref{it:cross-relabel-qi}, the directed form**: two
...
/-- **`thm:cross-relabel`\labelcref{it:cross-relabel-qi}, over the symmetric edge**:
