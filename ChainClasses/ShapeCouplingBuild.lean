...
The shrinking of `thm:shape-shrink`\labelcref{it:shape-shrink} is defined on the tail shapes
...
* `IsHalvingLevels`, `IsCascadeOn`: **the cascade over a partial shrinking**, with
  ...
  `cascadeOut_add_cascadeInOn` and `cascadeInOn_le_half` the two marginal clauses.
...
* `shrinkShape`, `capacity_of_mass_bounds`, `exists_capacity_shapePMF`: **`eq:capacity` at
  ...
  `thm:shape-mass`\labelcref{it:shape-mass-tail} for the source law and the point bound
  `thm:shape-mass`\labelcref{it:shape-mass-point} for the target law meet.
...
/-- **A target atom spends at most half its mass**, leaving the leftovers of the two sides
to be closed off. -/
theorem cascadeInOn_le_half (σ : T) : cascadeInOn dom shrink M σ ≤ M σ 0 / 2 := by
  have h1 := h.cascadeInOn_eq σ
  have h2 := h.toHalvingLevels.half_le_cascadeOut σ
  linarith
...
/-- **The shrinking of `thm:shape-shrink`\labelcref{it:shape-shrink} as a map**: a choice of
...
`thm:shape-mass`\labelcref{it:shape-mass-tail} for the source law and the point bound
`thm:shape-mass`\labelcref{it:shape-mass-point} for the target law meet, at the two
...
/-- The point bound `thm:shape-mass`\labelcref{it:shape-mass-point} at a constant common to
two laws. -/
...
`thm:shape-coupling`\labelcref{it:shape-coupling-eta} consumes: a charged pair is the
