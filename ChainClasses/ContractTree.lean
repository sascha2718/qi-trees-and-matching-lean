...
/-- The mark of a vertex is the index of its part, taken as a vertex of the
contracted tree. -/
lemma markOf_eq_idxOf_contract {w : Word} (hw : T.IsAddr w) :
    markOf s T w = (partList s T).idxOf (contract (s := s) hw).1 := rfl
