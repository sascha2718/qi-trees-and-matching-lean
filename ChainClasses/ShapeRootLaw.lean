...
* `survivalMeasure_neck_step`: **`thm:harris`\labelcref{it:harris-bushes} in the
  shape**, the decorated neck step.
* `neckRay_shift_base`, `IsHairySample.bushAt_zero`, `splitDepth_bushAt`,
...
/-- **`thm:harris`\labelcref{it:harris-bushes} in the shape**: conditioned on survival,
...
/-- The subtree along the neck of a hairy sample is again hairy. -/
lemma IsHairySample.bushAt_zero {c : Amb → ℕ} (hc : IsHairySample c)
    (hdeg : skeletonDegree c = 1) : IsHairySample (bushAt c 0) := by
  ...
  · exact survives_neck hroot
  · have hv' : Survives (shift c ([neckLetter c []] ++ v)) := by
      rwa [← shift_shift]
    obtain ⟨k, hk⟩ := hc.splits _ hv'
    refine ⟨k, ?_⟩
    rwa [shift_shift, neckRay_shift_base c [neckLetter c []] v k]
