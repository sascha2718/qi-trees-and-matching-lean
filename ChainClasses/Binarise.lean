/-
`thm:shape-shrink`\labelcref{it:shape-shrink} of `gw_classes_simple.tex`: the
...
* `Tri.lcrsNode`, `Tri.binForest`, `Tri.binRoot`, `Tri.binTree`: **the first
  move**, the binarisation, in its left-child right-sibling form read straight
  off the tree: a vertex of the contraction carries its first part below it at
  `false` and its next sibling part at `true`, an absent first child filled by
  a leaf.  `Tri.size_binTree_le` and `Tri.size_binTree_le_div` are its size
  clause, at most twice the vertices of the contraction.
* `Tri.part_append`, `Tri.urList`, `Tri.childList`: the cut read inside a
  ...
  `Tri.rawSize_le` counts a part at at most `2s-1` vertices,
  `Tri.length_rootForest_le_two_mul` and `Tri.length_childList_le_four_mul`
  count its children, at most `2s` and at most `4s` respectively; the second
  is what bounds the stretch of a path through a vertex the binarisation
  replaces.
...
* `markedQI_shape_shrink`: **the composition**, `thm:shape-shrink`
  \labelcref{it:shape-shrink} at the constant `2592 s²` and the size bound
...
/-! ### The binarisation, as a tree -/

/-- The left-child right-sibling node: a vertex of the binarised tree carries
its first child at `false` and its next sibling at `true`, an absent first
child being filled by a leaf. -/
def lcrsNode : Option Tri → Option Tri → Tri
  | none, none => .leaf
  | some l, none => .one l
  | none, some r => .two .leaf r
  | some l, some r => .two l r

@[simp] lemma lcrsNode_none_none : lcrsNode none none = .leaf := rfl
@[simp] lemma lcrsNode_some_none (l : Tri) : lcrsNode (some l) none = .one l := rfl
@[simp] lemma lcrsNode_none_some (r : Tri) : lcrsNode none (some r) = .two .leaf r := rfl
@[simp] lemma lcrsNode_some_some (l r : Tri) : lcrsNode (some l) (some r) = .two l r := rfl

/-- A node of the binarised tree contributes at most two vertices: its own, and the
leaf that fills an absent first child. -/
lemma size_lcrsNode_le (L R : Option Tri) :
    (lcrsNode L R).size ≤ 2 + optSize L + optSize R := by
  cases L <;> cases R <;> simp only [lcrsNode_none_none, lcrsNode_some_none,
    lcrsNode_none_some, lcrsNode_some_some, size, optSize_none, optSize_some] <;> omega

/-- **The first move**, on forests: the parts topped by the maximal cut
vertices of `t`, followed by the parts already collected in `rest`, strung out
as one right spine. -/
def binForest (s : ℕ) : Tri → Option Tri → Option Tri
  | .leaf, rest => if IsCut s .leaf then some (lcrsNode none rest) else rest
  | .one t, rest =>
      if IsCut s (.one t) then some (lcrsNode (binForest s t none) rest)
      else binForest s t rest
  | .two l r, rest =>
      if IsCut s (.two l r) then some (lcrsNode (binForest s l (binForest s r none)) rest)
      else binForest s l (binForest s r rest)

@[simp] lemma binForest_leaf (s : ℕ) (rest : Option Tri) :
    binForest s .leaf rest = if IsCut s .leaf then some (lcrsNode none rest) else rest := rfl

@[simp] lemma binForest_one (s : ℕ) (t : Tri) (rest : Option Tri) :
    binForest s (.one t) rest =
      if IsCut s (.one t) then some (lcrsNode (binForest s t none) rest)
      else binForest s t rest := rfl

@[simp] lemma binForest_two (s : ℕ) (l r : Tri) (rest : Option Tri) :
    binForest s (.two l r) rest =
      if IsCut s (.two l r) then some (lcrsNode (binForest s l (binForest s r none)) rest)
      else binForest s l (binForest s r rest) := rfl

/-- The parts strictly below the root, strung out as one right spine. -/
def binRoot (s : ℕ) : Tri → Option Tri
  | .leaf => none
  | .one t => binForest s t none
  | .two l r => binForest s l (binForest s r none)

@[simp] lemma binRoot_leaf (s : ℕ) : binRoot s .leaf = none := rfl
@[simp] lemma binRoot_one (s : ℕ) (t : Tri) : binRoot s (.one t) = binForest s t none := rfl
@[simp] lemma binRoot_two (s : ℕ) (l r : Tri) :
    binRoot s (.two l r) = binForest s l (binForest s r none) := rfl

/-- **The first move**: the binarised contraction, the root part carrying the
parts below it as its left spine. -/
def binTree (s : ℕ) (T : Tri) : Tri := graftOne (binRoot s T)

/-- Each part of the greedy cut contributes at most two vertices of the binarised
tree. -/
lemma optSize_binForest_le (s : ℕ) : ∀ (t : Tri) (rest : Option Tri),
    optSize (binForest s t rest) ≤ 2 * RTree.sizeF (cutForest s t) + optSize rest := by
  intro t
  induction t with
  | leaf =>
      intro rest
      by_cases h : IsCut s (.leaf : Tri)
      · rw [binForest_leaf, if_pos h, cutForest_leaf, if_pos h]
        have := size_lcrsNode_le none rest
        simp only [optSize_some, RTree.sizeF_cons, RTree.sizeF_nil, RTree.size_node,
          optSize_none] at *
        omega
      · rw [binForest_leaf, if_neg h, cutForest_leaf, if_neg h]
        simp
  | one t ih =>
      intro rest
      by_cases h : IsCut s (.one t)
      · rw [binForest_one, if_pos h, cutForest_one, if_pos h]
        have h1 := size_lcrsNode_le (binForest s t none) rest
        have h2 := ih none
        simp only [optSize_some, RTree.sizeF_cons, RTree.sizeF_nil, RTree.size_node,
          optSize_none] at *
        omega
      · rw [binForest_one, if_neg h, cutForest_one, if_neg h]
        exact ih rest
  | two l r ihl ihr =>
      intro rest
      by_cases h : IsCut s (.two l r)
      · rw [binForest_two, if_pos h, cutForest_two, if_pos h]
        have h1 := size_lcrsNode_le (binForest s l (binForest s r none)) rest
        have h2 := ihl (binForest s r none)
        have h3 := ihr none
        simp only [optSize_some, RTree.sizeF_cons, RTree.sizeF_nil, RTree.size_node,
          RTree.sizeF_append, optSize_none] at *
        omega
      · rw [binForest_two, if_neg h, cutForest_two, if_neg h]
        have h2 := ihl (binForest s r rest)
        have h3 := ihr rest
        simp only [RTree.sizeF_append] at *
        omega

lemma optSize_binRoot_le (s : ℕ) (T : Tri) :
    optSize (binRoot s T) ≤ 2 * RTree.sizeF (rootForest s T) := by
  cases T with
  | leaf => simp
  | one t =>
      have h := optSize_binForest_le s t none
      simpa using h
  | two l r =>
      have h1 := optSize_binForest_le s l (binForest s r none)
      have h2 := optSize_binForest_le s r none
      simp only [binRoot_two, rootForest_two, RTree.sizeF_append, optSize_none] at *
      omega

/-- **`thm:shape-shrink`\labelcref{it:shape-shrink}, the size of the first
move**: the binarised contraction has at most twice the vertices of the
contraction. -/
theorem size_binTree_le (s : ℕ) (T : Tri) :
    (binTree s T).size ≤ 2 * (contractTree s T).size := by
  have h := optSize_binRoot_le s T
  rw [binTree, size_graftOne, contractTree_eq, RTree.size_node]
  omega

/-- **`thm:shape-shrink`\labelcref{it:shape-shrink}**: at scale `s` the
binarised contraction of a tree of `n` vertices has at most `2(n/s+1)`
vertices. -/
theorem size_binTree_le_div {s : ℕ} (hs : 1 ≤ s) (T : Tri) :
    (binTree s T).size ≤ 2 * (T.size / s + 1) := by
  have h1 := size_binTree_le s T
  have h2 := size_contractTree_le_div (s := s) hs T
  omega
...
/-- A part has at most one child for each of its vertices, and one more: the
edges leaving the part are the child slots of its vertices less the edges
inside it. -/
lemma length_rootForest_le (s : ℕ) (t : Tri) :
    (rootForest s t).length ≤ rawSize s t + 1 := by
  cases t with
  | leaf => simp only [rootForest_leaf, List.length_nil]; omega
  | one t =>
      have h := length_cutForest_le s t
      simp only [rootForest_one, rawSize]
      omega
  | two l r =>
      have h1 := length_cutForest_le s l
      have h2 := length_cutForest_le s r
      simp only [rootForest_two, rawSize, List.length_append]
      omega

/-- **`thm:shape-shrink`, the branching of the contraction**: a part has at
most `2s` children, which is what bounds by `4s` the stretch of a path through
a vertex the binarisation replaces. -/
theorem length_rootForest_le_two_mul (s : ℕ) (hs : 1 ≤ s) (t : Tri) :
    (rootForest s t).length ≤ 2 * s := by
  have h1 := length_rootForest_le s t
  have h2 := rawSize_le s hs t
  omega
...
/-- **`thm:shape-shrink`\labelcref{it:shape-shrink}, the size of the second
...
/-- **`thm:shape-shrink`\labelcref{it:shape-shrink}, the second move**: the
...
lemma blk_injective : Function.Injective blk := by
  intro i j h
...
lemma pparent_mem_partList (T : Tri) {q : Word} (hq : T.IsAddr q) :
    pparent s T q ∈ partList s T := by
  refine (mem_partList_iff T).mpr ⟨?_, part_idem T _⟩
  exact T.isAddr_of_prefix (part_prefix T q.dropLast)
    (T.isAddr_of_prefix q.dropLast_prefix hq)
...
/-- **The stretch of one part edge**: a part sits at most `4s` steps below its
part-parent in the binarised tree. -/
lemma childIdx_lt {T : Tri} {q : Word} (hq : T.IsAddr q) (hpq : part s T q = q)
    (hne : q ≠ []) : childIdx s T q < (childList s T (pparent s T q)).length :=
  List.idxOf_lt_length_of_mem (mem_childList_pparent hq hpq hne)
...
/-- **`thm:shape-shrink`\labelcref{it:shape-shrink}, the first move**: the
...
/-- **`thm:shape-shrink`\labelcref{it:shape-shrink}**: at scale `s ≥ 1` every
...
/-- **`thm:shape-shrink`\labelcref{it:shape-shrink}, the choice of scale**: at
...
/-- **`thm:shape-shrink`\labelcref{it:shape-shrink}** at the paper's scale: for
