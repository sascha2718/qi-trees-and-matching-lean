...
* `toAddr`, `fromAddr`, `pattern_of_listPattern`: the presented addresses of the blob
  ...
  formula.
* `bArityPMF`, `bArityPMF_apply`: the presented arity law `ν̂` as a probability law.
* `chainLabW`, `bArityW`, `crossLabW`, `bArityW'`, `blobMeasure_map_encLab`,
  `coupledBlobMeasure_map_encLab`: **`thm:chain-cross`\labelcref{it:chain-cross-product},
...
* `matchedArityPMF`, `matchedArityPMF_succ`, `matchedArityPMF_support_floor`: the
  ...
  \labelcref{it:matched-support,it:matched-mass} of `thm:matched-presentation`.
* `exists_chain_scale`, `chart_pack`, `compTiltBound_le_of_floor`: the largeness
  conditions from some scale on, the chart clauses of
  `thm:chain-cross`\labelcref{it:chain-cross-core}, and the tilt bound from a uniform floor.
...
lemma bArityPMF_apply (θ : Offspring J) (hJN : J ≤ N) (hq : θ.extinction < 1)
    (hs1 : θ.skeletonWeight 1 < 1) (ρ : Measure σ) [IsProbabilityMeasure ρ] (R : ℕ)
    (ρr : BRule σ) (k : ℕ) : bArityPMF θ hJN hq hs1 ρ R ρr k = bArityLaw θ ρ R ρr k := rfl
...
/-- **`thm:chain-cross`\labelcref{it:chain-cross-product}, the identification on the `θ`
...
/-- The floor `c δ` of `thm:matched-presentation`\labelcref{it:matched-mass} at the
...
(`thm:matched-presentation`\labelcref{it:matched-support,it:matched-mass} on the constructed
...
/-- **`thm:chain-cross`\labelcref{it:chain-cross-core}, the chart clauses**: a chart of
...
of `thm:chain-cross`\labelcref{it:chain-cross-core} such that for every `δ ∈ (0, 1]` there
