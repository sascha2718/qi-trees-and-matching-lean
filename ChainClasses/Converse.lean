...
* `prefix_total`, `treeDist_append_left`: the ambient
  metric of `𝒩` as a metric, and its invariance inside a cone.
...
lemma prefix_total : ∀ p q u : Word, p <+: u → q <+: u → p <+: q ∨ q <+: p
  | [], _, _, _, _ => Or.inl List.nil_prefix
  | _ :: _, [], _, _, _ => Or.inr List.nil_prefix
  | _ :: _, _ :: _, [], hp, _ => absurd hp (by simp)
  | a :: p, b :: q, c :: u, hp, hq => by
      rw [List.cons_prefix_cons] at hp hq
      obtain ⟨rfl, hp⟩ := hp
      obtain ⟨rfl, hq⟩ := hq
      rcases prefix_total p q u hp hq with h | h
...
/-- The projection grows by the full label across a strict extension. -/
