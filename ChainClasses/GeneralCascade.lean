/-
`thm:merge`\labelcref{it:merge-split} of `matching_classes_general.tex`: the cascade
...
* `bitsL`, `length_bitsL`, `bitsL_inj`: the `L`-bit code of a letter, injective below
  `2^L`.
* `cascWord`, `length_cascWord`, `cascWord_injective`: the letter-by-letter encoding of
  a word, of length `L` times the length of the word.
...
* `treeDist_cascWord_le`, `le_treeDist_cascWord`: **`thm:merge`\labelcref{it:merge-split},
...
* `cascMap`, `cascade_isQIWith`, `cascade_quasiIsometric`, `cascade_isQIWith_clog`:
  **`thm:merge`\labelcref{it:merge-split}**, the cascade encoding is an
  `L`-quasi-isometry of the parent-child graphs in the sense of `def:qi`, at
  `L = ⌈log₂ N⌉` in particular.
* `treeDist_cascWord_child`, `cascade_internal`: the slot depths, each outgoing edge
  becoming a path of length exactly `L`, and every cascade vertex that is not a copy
  lying strictly inside one cascade.
* `shortWords`, `card_shortWords`, `cascade_split_card`: the cascade of one split has at
  most `2^L - 1` vertices besides its slots.
-/
...
/-- The encoding of a child: the edge to the `j`-th child becomes the path of length
`L` to its slot. -/
lemma treeDist_cascWord_child (L : ℕ) (w : BranchingProcess.Word N) (j : Fin N) :
    treeDist (cascWord L w) (cascWord L (w ++ [j])) = L := by
  rw [treeDist_of_prefix (cascWord_prefix L (List.prefix_append w [j]))]
  simp [Nat.mul_succ]
...
/-- **`thm:merge`\labelcref{it:merge-split}, the exact distance of diverging copies**:
...
/-- **`thm:merge`\labelcref{it:merge-split}, the upper bound**: copies are at most `L`
...
/-- **`thm:merge`\labelcref{it:merge-split}, the lower bound**: copies are at least as
...
/-- The encoding is injective once `1 ≤ L`: it does not shrink distances. -/
theorem cascWord_injective {L : ℕ} (hN : N ≤ 2 ^ L) (hL : 1 ≤ L) :
    Function.Injective (cascWord (N := N) L) := by
  intro u v h
  have h1 := le_treeDist_cascWord hN hL u v
  rw [h, treeDist_self, Nat.le_zero, BranchingProcess.treeDist_eq_zero_iff] at h1
  exact h1
...
/-- **The slot depths**: a cascade vertex that is no copy lies strictly inside the cascade
of one split, at depth between `1` and `L - 1` below the copy of that split. -/
theorem cascade_internal (L : ℕ) {T : BranchingProcess.Word N → Prop} (hT : PrefixClosedN T)
    {x : Word} (hx : cascSet L T x) (hcopy : ∀ w, T w → x ≠ cascWord L w) :
    ∃ w, T w ∧ cascWord L w <+: x ∧ 0 < treeDist (cascWord L w) x ∧
      treeDist (cascWord L w) x ≤ L - 1 := by
  obtain ⟨w, hw, hpre, hle⟩ := exists_copy_near L hT hx
  refine ⟨w, hw, hpre, Nat.pos_of_ne_zero fun h => hcopy w hw ?_, hle⟩
  exact (treeDist_eq_zero_iff.mp h).symm
...
/-- **`thm:merge`\labelcref{it:merge-split}.** Replacing every split of a prefix-closed
...
/-- **`thm:merge`\labelcref{it:merge-split}, as quasi-isometry of graphs.** -/
...
/-- **`thm:merge`\labelcref{it:merge-split}, at the depth `⌈log₂ N⌉`**: for `2 ≤ N`, the
...
/-! ### The count of one cascade -/

/-- The binary words of length below `L`: the possible internal vertices of one cascade
of depth `L`. -/
def shortWords (L : ℕ) : Finset Word :=
  (Finset.range L).biUnion fun n => (Finset.univ : Finset (Fin n → Bool)).image List.ofFn

lemma mem_shortWords {L : ℕ} {x : Word} : x ∈ shortWords L ↔ x.length < L := by
  simp only [shortWords, Finset.mem_biUnion, Finset.mem_range, Finset.mem_image,
    Finset.mem_univ, true_and]
  constructor
  · rintro ⟨n, hn, f, rfl⟩
    simpa using hn
  · intro h
    exact ⟨x.length, h, x.get, List.ofFn_get x⟩

/-- There are `2^L - 1` binary words of length below `L`. -/
lemma card_shortWords (L : ℕ) : (shortWords L).card = 2 ^ L - 1 := by
  rw [shortWords, Finset.card_biUnion]
  · simp_rw [Finset.card_image_of_injective _ List.ofFn_injective, Finset.card_univ,
      Fintype.card_fun, Fintype.card_bool, Fintype.card_fin]
    rw [Nat.geomSum_eq (le_refl 2) L]
    simp
  · intro n _ m _ hnm
    change Disjoint _ _
    rw [Finset.disjoint_left]
    rintro x hx hx'
    simp only [Finset.mem_image, Finset.mem_univ, true_and] at hx hx'
    obtain ⟨f, rfl⟩ := hx
    obtain ⟨g, hg⟩ := hx'
...
/-- The vertices of one split of arity `k`: the root and its `k` children. -/
def splitSet (k : ℕ) (w : BranchingProcess.Word k) : Prop := w.length ≤ 1

lemma prefixClosedN_splitSet (k : ℕ) : PrefixClosedN (splitSet k) :=
  fun _ _ huv hv => huv.length_le.trans hv

/-- A vertex of the cascade of one split is a slot or a word of length below `L`. -/
lemma cascSet_splitSet {L k : ℕ} (hL : 1 ≤ L) {x : Word} (hx : cascSet L (splitSet k) x) :
    (∃ j : Fin k, x = bitsL L j) ∨ x.length < L := by
  obtain ⟨w, hw, hxw⟩ := hx
  match w, hw with
  | [], _ =>
      rw [cascWord_nil, List.prefix_nil] at hxw
      subst hxw
      exact Or.inr (by simp; omega)
  | [j], _ =>
      rw [cascWord_cons, cascWord_nil, List.append_nil] at hxw
      rcases eq_or_ne x.length L with h | h
      · exact Or.inl ⟨j, hxw.eq_of_length (by rw [h, length_bitsL])⟩
      · exact Or.inr (lt_of_le_of_ne (by simpa using hxw.length_le) h)
  | _ :: _ :: _, hw => simp [splitSet] at hw

open Classical in
/-- **`thm:merge`\labelcref{it:merge-split}, the count**: the cascade of one split of
arity `k` has at most `2^L - 1` vertices other than its `k` slots, the root included. -/
theorem cascade_split_card (L k : ℕ) :
    ((shortWords L).filter (cascSet L (splitSet k))).card ≤ 2 ^ L - 1 :=
  (Finset.card_filter_le _ _).trans (card_shortWords L).le

open Classical in
/-- Every non-slot vertex of the cascade of one split is counted by
`cascade_split_card`. -/
lemma mem_filter_shortWords_of_cascSet {L k : ℕ} (hL : 1 ≤ L) {x : Word}
    (hx : cascSet L (splitSet k) x) (hslot : ∀ j : Fin k, x ≠ bitsL L j) :
    x ∈ (shortWords L).filter (cascSet L (splitSet k)) := by
  rw [Finset.mem_filter, mem_shortWords]
  refine ⟨?_, hx⟩
  rcases cascSet_splitSet hL hx with ⟨j, hj⟩ | h
  · exact absurd hj (hslot j)
  · exact h
