...
* `levelMap`: `ℓ_D = Nat.log D`, with the fibre description `level_eq_iff`
  (the classes `𝒞_k` of `thm:chain-classes`).
...
* The quantised law: `qF a D k = a^{D^k-1} - a^{D^{k+1}-1}`, the closed form
  ...
  (the summation in `thm:chain-classes`); `qF_zero` and `qF_le` are
...
/-- The fibres of the level map are the classes `𝒞_k` of `thm:chain-classes`:
...
/-- The within-class ratio clause of `thm:chain-classes`: two members of one
...
`q_k = a^{D^k-1} - a^{D^{k+1}-1}` of the quantised law `p^{(D)}`. -/
...
/-- The first clause of **`thm:quantised-law`**: `p^{(D)}_0 = 1 - a^{D-1}`. -/
lemma qF_zero (a : ℝ) (D : ℕ) : qF a D 0 = 1 - a ^ (D - 1) := by
  simp [qF, tailB]

/-- The second clause of **`thm:quantised-law`**: `p^{(D)}_k ≤ a^{D^k - 1}`. -/
