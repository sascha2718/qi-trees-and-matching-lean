...
* `hairy_rate`: **`eq:hairy-rate` for the assemblies**, the failure probability
  ...
  `exists_scale_of_exp_rates` is the last display of the proof, rates
  `16e^{-c₃D²}` beating every threshold, and `hairy_ae` the almost sure
  statement it gives.
...
/-- **The last display of `thm:hairy`**: failure rates `16e^{-c₃D²}` along the
scales beat every threshold, so the events of `eq:hairy-rate` exhaust the
sample space.  This is `hairy_rate_to_one` read on the two-sample space. -/
lemma exists_scale_of_exp_rates {Ω : Type*} [MeasurableSpace Ω] (P : Measure Ω) {c : ℝ}
    (hc : 0 < c) {E : ℝ → Set Ω}
    (hrate : ∀ D : ℕ, ∃ L : ℝ, P (E L) ≤ ENNReal.ofReal (16 * Real.exp (-(c * (D : ℝ) ^ 2))))
    (ε : ℝ≥0∞) (hε : 0 < ε) : ∃ L : ℝ, P (E L) ≤ ε := by
  rcases eq_or_ne ε ⊤ with rfl | hεtop
  · obtain ⟨L, -⟩ := hrate 0
    exact ⟨L, le_top⟩
  · have hpos : 0 < ε.toReal := ENNReal.toReal_pos hε.ne' hεtop
    obtain ⟨D₀, hD₀⟩ := hairy_rate_to_one hc hpos
    obtain ⟨L, hL⟩ := hrate D₀
    refine ⟨L, hL.trans ?_⟩
    calc ENNReal.ofReal (16 * Real.exp (-(c * (D₀ : ℝ) ^ 2)))
        ≤ ENNReal.ofReal ε.toReal := ENNReal.ofReal_le_ofReal (hD₀ D₀ le_rfl).le
      _ = ε := ENNReal.ofReal_toReal hεtop

/-- **The almost sure statement of `thm:hairy` for the assemblies**: if the
failure probability of `eq:hairy-rate` can be made arbitrarily small, then
almost surely the two assemblies are quasi-isometric. -/
theorem hairy_ae (θ θ' : Offspring 2)
    (hscale : ∀ ε : ℝ≥0∞, 0 < ε → ∃ L : ℝ, twoHairyMeasure θ θ'
      {ω | ¬ ∃ f : Assembly (shapeAt ω.1.1) → Assembly (shapeAt ω.2.1), IsQIMap L f} ≤ ε) :
    twoHairyMeasure θ θ'
      {ω | ¬ ∃ (L : ℝ) (f : Assembly (shapeAt ω.1.1) → Assembly (shapeAt ω.2.1)),
        IsQIMap L f} = 0 := by
  ...
  have hsub : {ω : HairySample × HairySample |
        ¬ ∃ (L : ℝ) (f : Assembly (shapeAt ω.1.1) → Assembly (shapeAt ω.2.1)), IsQIMap L f}
      ⊆ {ω | ¬ ∃ f : Assembly (shapeAt ω.1.1) → Assembly (shapeAt ω.2.1), IsQIMap L f} := by
    rintro ω hω ⟨f, hf⟩
    exact hω ⟨L, f, hf⟩
  calc twoHairyMeasure θ θ' {ω | ¬ ∃ (L : ℝ)
          (f : Assembly (shapeAt ω.1.1) → Assembly (shapeAt ω.2.1)), IsQIMap L f}
      ≤ twoHairyMeasure θ θ'
          {ω | ¬ ∃ f : Assembly (shapeAt ω.1.1) → Assembly (shapeAt ω.2.1),
            IsQIMap L f} := measure_mono hsub
    _ ≤ (ε : ℝ≥0∞) := hL
