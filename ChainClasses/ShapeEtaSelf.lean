...
`thm:shape-mass`\labelcref{it:shape-mass-tail}, `ShapeShrink` the collapse of a shape of
small diameter and `Binarise` the shrinking of
`thm:shape-shrink`\labelcref{it:shape-shrink}.  What is left is the geometry that turns
...
* `shapePMF_eq_tsum`, `shapeMass_le_shapePMF`, `tsum_fibre_size_shapePMF`: **the label law
  ...
  `thm:shape-shrink`\labelcref{it:shape-fix}, which only adds mass and carries the size
  fibres onto the law of `fixSizeMass`.
* `Tri.count_ne_one_of_full`, `bushPointBound_full`,
  `ofReal_pow_size_le_shapeMass_supported`: **`thm:shape-mass`\labelcref{it:shape-mass-point}
...
/-- **`thm:shape-mass`\labelcref{it:shape-mass-point}, the bush input in the support**: a
...
/-- **`thm:shape-mass`\labelcref{it:shape-mass-point} in the support**: the mass of a
...
/-- The constant `p` of `thm:shape-mass`\labelcref{it:shape-mass-point} in the support:
...
/-- **`thm:shape-mass`\labelcref{it:shape-mass-point}** at the constant `shapeWeight`. -/
...
`thm:shape-shrink`\labelcref{it:shape-shrink} is taken at the scale `3D²`, so that its
...
/-- **`thm:shape-mass`\labelcref{it:shape-mass-tail} at the label law**: the labels of
...
/-- **`eq:hairy-rate`, one law, at the rate of `eq:shape-eta`**: the failure probability
decays exponentially in `D²`. -/
theorem hairy_rate_shape_tree_exp (θ : Offspring 2) (hq : θ.extinction < 1)
    (hq0 : 0 < θ.extinction) (h2 : 0 < θ 2) :
    ∃ c : ℝ, 0 < c ∧ ∃ D₀ : ℕ, ∀ D : ℕ, D₀ ≤ D →
      twoHairyMeasure θ θ
          {ω | ¬ ∃ F : {v : Amb // v ∈ sample ω.1.1} → {v : Amb // v ∈ sample ω.2.1},
            IsSampleQI (8 * 729 ^ 2 * (D : ℝ) ^ 8) F}
        ≤ ENNReal.ofReal (16 * Real.exp (-(c * (D : ℝ) ^ 2))) := by
  obtain ⟨c, hc, D₁, hD₁⟩ := exists_etaG_shapeNet_le θ hq hq0 h2
  obtain ⟨D₀, hD₀⟩ := exists_etaG_shapeNet_small θ hq hq0 h2
  refine ⟨c, hc, D₀ + D₁ + 1, fun D hD ↦ ?_⟩
  have hDR : (1 : ℝ) ≤ (D : ℝ) := by exact_mod_cast (by omega : 1 ≤ D)
  refine (hairy_rate_shape_tree_self θ hq hq0 h2 hDR (hD₀ D (by omega))).trans ?_
  calc (16 : ℝ≥0∞) * etaG (shapePMF θ hq hq0 h2) (shapeNet (D : ℝ))
      ≤ 16 * ENNReal.ofReal (Real.exp (-(c * (D : ℝ) ^ 2))) := by
        gcongr
        exact hD₁ D (by omega)
    _ = ENNReal.ofReal (16 * Real.exp (-(c * (D : ℝ) ^ 2))) := by
