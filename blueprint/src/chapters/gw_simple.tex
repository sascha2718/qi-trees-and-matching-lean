...
\leanok
\uses{thm:shape-iid}
When $\theta_1'>0$ all three weights of the conjugate law are positive, so every neck length
occurs and a neck vertex is bare or carries any prescribed finite bush with positive probability.
The support fix replaces the forbidden pieces by admissible ones of at most twice the size, and
composing the contraction, the binarisation and the neck construction meets the stated scale.
\end{proof}
...
Build a law $q$ on $\cS\times\cS$ with marginals $\mu$ and $\mu'$ supported on pairs of
$9D^3$-comparable shapes, by a cascade pairing that matches mass from the largest shapes
downwards. Its support is the vertex set of the common label graph, and the capacity estimate
bounds the resulting potential exactly as in \cref{thm:shape-eta}.
\end{proof}
...
\begin{proof}
\leanok
Consecutive vertices of $[x,y]$ have images at most $2D$ apart. An interior vertex $m$ of
$[\psi(x),\psi(y)]$ separates $\psi(x)$ from $\psi(y)$ in the tree $T'$, so some consecutive
pair of images lies on opposite sides of $m$, and $m$ is within $D$ of one of them.
\end{proof}
...
Let $P$ be a hair of depth $h>D(D+R)+D^2$ at $c$, and $x\in P$ at distance $h$ from
$c$. The image of the geodesic $[c,x]$ stays within $D$ of the geodesic joining the images by
\cref{thm:tree-stability}, while the image of $x$ lies within $R$ of a line. Following that line
back past $\psi(c)$ produces a vertex of $T$ off the hair and too close to $x$, contradicting
the depth.
\end{proof}
...
\leanok
\uses{thm:tree-stability}
Let $v$ be the common initial vertex and take $a_1,a_2,a_3$ at distance $r>2D^2$ from
$v$ along the three rays. Two of the three images lie on the same side of $\psi(v)$ in
$\bbN_0$, so the geodesic between them passes within $D$ of $\psi(v)$; pulling back by
\cref{thm:tree-stability} puts a vertex of the corresponding geodesic in $T$ near $v$, which the
rays exclude.
\end{proof}
...
$\set{1,2}$, so almost surely it branches infinitely often, which supplies the three rays. In the
chain regime every vertex sits on the line through its chain. In the bushy regime the dying bushes
recur at every depth along the skeleton, giving hairs of unbounded depth.
\end{proof}
...
\leanok
\uses{thm:tree-stability}
Suppose $\psi\colon\cT\to\bbB$ is a $D$-quasi-isometry. A ball of $\bbB$ around
$\psi(v)$ is covered by the images of a ball of $\cT$ around $v$ of comparable radius, so a
ball of $\cT$ isometric to a segment of $\bbZ$ forces the corresponding ball of $\bbB$ to have
at most linearly many vertices. Taking $\ell$ large contradicts the exponential growth of
$\bbB$.
\end{proof}
