...
\leanok
\uses{thm:harris}
Each child survives independently with probability $1-q$, so the number of surviving children
of a vertex conditioned to survive has the transformed generating function. That transform is the
reduced law, and the children that die found independent bushes under the conjugate law.
\end{proof}
...
\leanok
\uses{thm:harris-general}
A vertex has total offspring $k+r$ with exactly $k$ surviving with probability
$\theta_{k+r}\binom{k+r}{k}(1-q)^kq^{\,r}$, and conditioning on being a split divides by
$1-\widetilde\theta_1$. For $J\geq3$ the binomial coefficient couples $k$ and $r$, so no
...
\begin{proof}
\leanok
The neck length is geometric and independent of the arity of the terminating split, so the
conditioning leaves it unchanged, and the neck decorations are the conjugate bushes for the same
reason. Only the exit bouquet feels the conditioning, and its law is the $r$-marginal of
\cref{thm:split-joint} at fixed $k$.
\end{proof}
...
\begin{proof}
\leanok
The neck lengths are independent of the arity field and of each other, and the decorations are
conditionally independent given the skeleton. Assembling the two gives shapes that are conditionally
independent with the law attached to their own arity.
\end{proof}
...
\begin{proof}
\uses{thm:shape-mass}
Give $\mu_k$ the explicit form of \cref{thm:conditional-explicit}. The size is dominated by
the compound sum of \cref{thm:shape-mass} together with at most $J-2$ further independent total
progenies under the conjugate law, which keeps the exponential tail; the pointwise lower bound is
again a product of boundedly many fixed positive weights.
\end{proof}
...
\begin{proof}
\leanok
\uses{thm:conditional-iid, thm:mass-uniform}
The uniform mass bounds supply what the cascade of \cref{thm:shape-coupling} needs. Build its
two shrinking maps at scale $\floor{\sqrt{D/(216J^2)}}$, after fixing each shape into the
support, and take $\mu_D$ to be the common pushforward. The potential bound is the entropy count
of \cref{thm:dilution} against the mass bounds.
\end{proof}
...
Conditionally on the arity field the shapes are independent with $S(w)\sim\mu_{k(w)}$, so the
labels are conditionally independent with the single law $\mu_D$, whatever the arity field. That
law is free of the arity, so averaging gives independence of the pair and the product form.
\end{proof}
...
\leanok
\uses{thm:relabel}
Take $\mu_D=\mu\circ\rep_D^{-1}$ and the maps of \cref{thm:relabel} on the unprimed side. On
the primed side, sample a partner from the coupling of $\mu_k$ with $\mu$ by the uniform
variable and relabel by its representative. Both sides then push forward to $\mu_D$.
\end{proof}
...
\begin{proof}
\leanok
The realisations at $s$ and at $t$ are subtrees joined by the edge from $s$ to the entry
at $t$; their union along that edge is a merged shape with the combined exit list, of arity
$k_1+k_2-1$. Addressing the exits by words of length $\ceil{\log_2J}$ realises the contraction
as a quasi-isometry with that constant.
\end{proof}
...
\begin{proof}
\leanok
Assume $d=1$ by dividing the generators. B\'ezout gives a representation of $1$ as an
integer combination of the generators, and clearing signs exhibits two consecutive integers in
$\Lambda$; every integer beyond their product is then a nonnegative combination of them.
\end{proof}
...
\leanok
\uses{thm:harris-general}
Descend from the vertex above $w$. The offspring numbers are i.i.d.\ with law $\theta$, and
since $\theta_0=0$ every child survives, so a vertex is a neck vertex exactly when it has one
child. The event therefore reads off $r-1$ unary steps followed by a split of arity $j$, giving
the product $\theta_1^{\,r-1}\theta_j$.
\end{proof}
...
The pairs are i.i.d.\ with independent coordinates by \cref{thm:chain-independence}, and
$\ell_D$ acts on the first coordinate alone, so the labelled pairs are i.i.d.\ with the product of
the quantised geometric law and the arity law.
\end{proof}
...
\begin{proof}
\leanok
\uses{thm:chain-product-form}
The rule inspects only $m(w)$, $k(w)$, the uniform variable, and a bounded prefix of the
next neck, and it absorbs everything it inspects. The branching property therefore leaves the
unexplored subtrees independent, so the presented pairs are i.i.d., and summing the absorption
probabilities gives the presented arity law.
\end{proof}
...
\leanok
\uses{thm:merge}
Send the vertices of the inner neck to the far end of the outer neck and fix everything else.
Distances never increase and decrease by at most $m(w_1)\leq R$, so the map is an $R$-marked
quasi-isometry, and it is onto with each mark moved by at most $R$.
\end{proof}
...
\begin{proof}
\leanok
\uses{thm:cluster-law}
Expose the blobs successively. At each new cluster root the rule has absorbed every vertex it
inspected on the way there, so the unexplored subtrees are independent by the branching property.
The presented arity is then one more than a sum of shifts drawn by the rule, whose law is the
corresponding mixture of convolution powers of $P$.
\end{proof}
...
\begin{proof}
\leanok
A least nonzero element of a submonoid of $\bbN_0$ is not a sum of two nonzero elements, both
of which would be at least as large, so it lies in every generating set. Here the generating set is
the set of shifts, that is the support of the reduced law minus one.
\end{proof}
...
A cluster of splits of arities $k_1,\dots,k_n$ has $\sum_i(k_i-1)+1$ outgoing edges, since
each split contributes its arity and each absorbed split consumes one edge of its parent. That is
$s+1$ for $s$ a sum of generators, and every element of $\Lambda_\theta$ arises this way.
\end{proof}
...
Track the lag between the two totals. After the first two draws the lag stays in a finite
window determined by the two maximal increments, so the lag is a finite-state chain. Within each
block of $H$ draws a correction word returns it to zero with probability at least $q$, which
gives the exponential tail.
\end{proof}
...
Build the rule on one side and mirror it. At the start of each blob the rule chooses, by an
independent uniform variable, how many shifts to accumulate before stopping, capped so the presented
shift stays within a bounded window of the common renewal of \cref{thm:common-renewal}. The
resulting presented pairs are i.i.d.\ with the same arity law on both sides.
\end{proof}
