...
\leanok
\uses{def:potential}
Fix a point of the product support. Under the product relation the good degree is the
product of the two good degrees, so the bad degree is $q_1+(1-q_1)q_2$. The splitting bound for
$\phi_\alpha$ separates the two coordinates, and summing against the product measure gives the
bound at exponent $2\alpha$.
\end{proof}
...
The finite-height bounds are uniform in $h$ and the matching relations are consistent under
restriction, so a compactness argument over the levels produces a single automorphism matching
every vertex. The measure layer carries the bound through the projective limit.
\end{proof}
...
\begin{proof}
\leanok
Split the sum defining $\eta_{G,\alpha}$ at the distinguished vertex. The term at $0$ is
bounded by the mass outside $B_G(0,1)$ divided by $(1-\varepsilon_0)^\alpha$, and every other
term by $\mu(v)/\mu(B_G(v,1))^\alpha$, since $1-b(v)\leq1$.
\end{proof}
...
The ratios $a_{j+1}/a_j\leq e^{-20}<\tfrac12$ make the tail sums geometric, so $p_0>\tfrac12$
and the two neighbouring masses exceed $\tfrac12$. Local dominance then bounds the potential by
$4e^{-D(D-5/2)}$, which is below $10^{-4}$ for $D\geq5$.
\end{proof}
